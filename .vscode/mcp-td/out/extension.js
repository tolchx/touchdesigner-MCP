...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mcp-td.start', startServer), vscode.commands.registerCommand('mcp-td.stop', stopServer), vscode.commands.registerCommand('mcp-td.status', showStatus), vscode.commands.registerCommand('mcp-td.execute', executePython), vscode.commands.registerCommand('mcp-td.listTools', listTools));
}
function deactivate() {
    stopServer();
}
...
async function startServer() {
    ...
    if (!fs.existsSync(indexJs)) {
        vscode.window.showErrorMessage(`MCP server not found at ${indexJs}. Run 'npm run build' in the workspace root first.`);
        return;
    }
    const config = vscode.workspace.getConfiguration('mcp-td');
    const host = config.get('host', 'localhost');
    const port = config.get('port', 44444);
    const env = {
        ...process.env,
        TDAPI_HOST: host,
...
async function listTools() {
    ...
    if (serverProcess) {
        ...
        if (serverProcess.stdin) {
            ...
            let responseData = '';
            const onData = (data) => { responseData += data.toString(); };
            serverProcess.stdout?.on('data', onData);
            serverProcess.stdin.write(request + '\n');
            ...
            if (responseData) {
                try {
                    ...
                    if (parsed.result?.tools) {
                        const items = parsed.result.tools.map((t) => ({
                            label: t.name,
