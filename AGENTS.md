...
16. **Post-build wiring check (verificado en vivo en 2025.31760)**: después de CUALQUIER script que cree/cablee red, verificá el cableado con `GET /connections` (aristas reales `{from, fromPath, to, toPath, input}`; `total` = aristas) — NO confíes en el output del build. Compará aristas como SETS de `(from, to, input_index)` contra lo esperado: detecta wire faltante, output faltante E input index equivocado (mergePOP/copyPOP), y nombra la arista exacta que falta. Protocolo reproducible: `python scripts/live/post_build_wiring_check.py --path /project1/mi_red --expect "srcA->nz:0,nz->mg:0,srcB->mg:1,mg->out:0"` (selftest con 1 positivo + 3 negativos detectados: `--selftest`). Por qué: un build puede reportar éxito con la mitad de los wires (try/except que traga, connect dinámico que reemplaza un slot) — la única fuente de verdad del cableado es el propio TD vía `/connections` (ítem 38). Es el paso que sigue a la regla 4 (`/verify` errors) y la regla 5 (`n.errors()`): estructura primero, luego errores.
