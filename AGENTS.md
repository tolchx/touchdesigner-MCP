...
10h. **Solo POPs ok_con_input en redes generadas**: la base de conocimiento (`pop_operators.json`) lleva la categoría de evidencia de la matriz en vivo por tipo. El planner penaliza y advierte sobre POPs que NO son `ok_con_input` (21 tipos: cross-family, hardware, salida, o que requieren config especial); se aceptan solo si el pedido los nombra explícitamente. Consultá `isRecommendedForNetworks(type)` antes de sugerir un POP en una red nueva.
