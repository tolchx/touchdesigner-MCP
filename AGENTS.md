...
10k. **POP→imagen**: `/screenshot` solo captura TOPs; no existe operador POP→TOP. Camino: POP dentro de geometryCOMP + renderTOP con params `camera`/`geometry` (strings, no wires: geometryCOMP no tiene output connectors). Para evidencia numérica usar `points('P')[i]` y `bounds()` (los attrs custom GLSL pueden no ser CPU-readable).
