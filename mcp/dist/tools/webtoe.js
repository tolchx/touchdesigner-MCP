...
import { getCatalogEntry, listByFamily, getCreationDefaults } from "../catalogManager.js";
 ...
 */
function generateTopShader(opType, backend) {
 ...
 */
async function generateOpSpec(opType) {
 ...
 */
function buildWebtoeJson(prompt, options = {}) {
    ...
    if (nodes.length > 0) {
        nodes[nodes.length - 1].flags = { display: true };
    }
    return {
        app: "webtoe",
        version: 1,
...
// ─── MCP Tools ─────────────────────────────────────────────────────────────
export function registerWebtoeTools(server, client) {
    // ---------------------------------------------------------------------------
    // wt_generate_op — Generate OpSpec + shader for any TD operator
    // ---------------------------------------------------------------------------
    server.registerTool("wt_generate_op", {
        title: "Generate WebToe Operator Spec",
        description: "Generates a complete WebToe OpSpec for any TouchDesigner operator type. " +
        ...
        try {
            ...
            return ok({
                ...
                is_implementable: !result.spec.isStub,
                note: result.spec.isStub
    ...
    // ---------------------------------------------------------------------------
    server.registerTool("wt_build_network", {
        title: "Build WebToe Network",
        description: "Convert a natural language description into a complete .webtoe.json file. " +
        ...
        try {
            ...
            return ok({
                graph,
                size: {
                    nodes: graph.root.nodes.length,
                    wires: graph.root.wires.length,
                },
                meta_templates: templates.slice(0, 3).map((t) => t.name).filter(Boolean),
                meta_recipes: recipes.slice(0, 3).map(r => r.name).filter(Boolean),
                resolved_operators: resolvePrompt(prompt).allOperatorTypes.map((o) => o.opType),
    ...
    // ---------------------------------------------------------------------------
    server.registerTool("wt_list_gaps", {
        title: "List WebToe Coverage Gaps",
        description: "List operators in our TD research corpus that WebToe doesn't yet implement. " +
        ...
        try {
            ...
            for (const fam of families) {
                ...
                if (gaps.length > 0)
                    catalogGaps[fam] = gaps.slice(0, limit);
            }
            const total = Object.values(catalogGaps).reduce((a, b) => a + b.length, 0);
            return ok({
                total_gaps: total,
                by_family: Object.fromEntries(Object.entries(catalogGaps).map(([f, ops]) => [f, { count: ops.length, ops }])),
    ...
    // ---------------------------------------------------------------------------
    server.registerTool("wt_resolve_prompt", {
        title: "Resolve WebToe Operators",
        description: "Convert natural language to WebToe operator types. " +
        ...
        try {
            const resolved = resolvePrompt(prompt);
            const wtOperators = resolved.allOperatorTypes.slice(0, limit ?? 5).map((op) => {
