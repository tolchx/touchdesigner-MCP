 ...
 */
function scoreDocument(content, queryTerms) {
 ...
 */
function extractRelevantSection(content, queryTerms, maxChars = 3000) {
 ...
 */
export function registerKnowledgeQueryTool(server, client) {
    server.registerTool("td_knowledge_query", {
        title: "Query POP Knowledge Base",
        description: "Search across the comprehensive POP knowledge base (Docs/110-118) for relevant documentation on TouchDesigner POPs. Covers fundamentals, GLSL, forces, physics, rendering, workflows, new features, operator parameters, troubleshooting, and more. Returns the most relevant document sections matching the query.",
        inputSchema: {
            query: z
                .string()
                .describe("Search query describing what POP knowledge you need (e.g. 'GLSL particle shader', 'boids flocking', 'Particle POP parameters')"),
            doc: z
                .number()
                .int()
                .min(110)
                .max(118)
                .optional()
                .describe("Optional specific document number (110-118) to search within. If omitted, searches all documents."),
            limit: z
                .number()
                .int()
                .min(1)
        ...
        try {
            ...
            if (topResults.length === 0) {
                return ok({
                    found: false,
                    query,
                    message: "No relevant knowledge found for this query. Try broader search terms or specify a document number (110-118).",
                    availableDocs: DOC_INDEX.map((d) => `Docs/${d.num}: ${d.title}`),
                });
            }
            return ok({
                found: true,
