 ...
 */
function buildOperatorExistsCode(paths) {
 ...
 */
function buildConnectionCheckCode(connections) {
 ...
 */
function buildFeedbackLoopCode(paths) {
    const safePaths = paths.map((p) => p.replace(/'/g, "\\'"));
    return `
import json
results = []
 ...
 */
function buildAttributeCheckCode(checks) {
...
// ─── Tool Registration ───────────────────────────────────────────────────────
export function registerSmokeTestTools(server, client) {
    // ---------------------------------------------------------------------------
    // td_smoke_test
    // ---------------------------------------------------------------------------
    server.registerTool("td_smoke_test", {
        title: "Functional Smoke Test",
        description: "Run functional smoke tests on a TouchDesigner network after building. " +
        ...
        inputSchema: {
            ...
            expected_operators: z
                .array(z.string())
                .optional()
                .describe("List of operator paths that must exist (e.g. ['/project1/mySystem/noise1', '/project1/mySystem/null1'])"),
            ...
            check_feedback_loops: z
                .array(z.string())
                .optional()
                .describe("Operator paths where feedback loops should exist (e.g. feedback POPs, nullPOP feedback targets)"),
            check_attributes: z
                .array(z.object({
    ...
    }, async ({ path: rootPath, expected_operators, expected_connections, check_feedback_loops, check_attributes, }) => {
        try {
            const result = {
                ok: true,
                path: rootPath,
            ...
            if (opPaths.length > 0) {
                ...
                try {
                    const opResults = JSON.parse(execResult.stdout || "[]");
                    ...
                    for (const r of opResults) {
                        if (r.exists)
                            ...
                            result.ok = false;
                            result.fixSuggestions.push(`Create missing operator at '${r.path}' — operator was expected but not found`);
                        }
                    }
                }
                catch {
            ...
            if (connections.length > 0) {
                ...
                const execResult = await client.execute(code, "/");
                try {
                    const connResults = JSON.parse(execResult.stdout || "[]");
                    ...
                    for (const r of connResults) {
                        if (r.connected)
                            ...
                            result.ok = false;
                            result.fixSuggestions.push(`Fix connection: ${r.sourcePath} → ${r.targetPath} input[${r.inputIndex}]: ${r.error || "not connected"}`);
                        }
                    }
                }
                catch {
            ...
            if (loopPaths.length > 0) {
                ...
                const execResult = await client.execute(code, "/");
                try {
                    const loopResults = JSON.parse(execResult.stdout || "[]");
                    ...
                    for (const r of loopResults) {
                        if (r.hasLoop)
                            ...
                            result.ok = false;
                            result.fixSuggestions.push(`Feedback loop broken at '${r.path}' — no upstream cycle detected`);
                        }
                    }
                }
                catch {
            ...
            if (attrChecks.length > 0) {
                const code = buildAttributeCheckCode(attrChecks.map((c) => ({ path: c.path, attributeName: c.attribute })));
                const execResult = await client.execute(code, "/");
                try {
                    const attrResults = JSON.parse(execResult.stdout || "[]");
                    ...
                    for (const r of attrResults) {
                        if (r.present)
                            ...
                            result.ok = false;
                            result.fixSuggestions.push(`Missing attribute '${r.attributeName}' on '${r.path}'${r.error ? `: ${r.error}` : ""}`);
                        }
                    }
                }
                catch {
...
`;
                    ...
                    const pops = JSON.parse(scanResult.stdout || "[]");
                    const autoAttrs = [];
                    ...
                    if (autoAttrs.length > 0) {
                        ...
                        try {
                            const autoResults = JSON.parse(autoResult.stdout || "[]");
                            ...
                            for (const r of autoResults) {
                                if (r.present)
                                    ...
                                    result.tiers.attributes.failed++;
                                    result.ok = false;
                                    result.fixSuggestions.push(`Missing POP attribute '${r.attributeName}' on '${r.path}' — add attributePOP to initialize`);
                                }
                            }
                        }
            ...
            // ── Summary ──
            const totalPassed = result.tiers.operatorExists.passed +
                result.tiers.connections.passed +
                result.tiers.feedbackLoops.passed +
                result.tiers.attributes.passed;
            const totalFailed = result.tiers.operatorExists.failed +
                result.tiers.connections.failed +
                result.tiers.feedbackLoops.failed +
                result.tiers.attributes.failed;
            const totalChecks = result.tiers.operatorExists.total +
