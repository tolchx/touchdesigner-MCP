 ...
 */
function buildAutoFixCode(targetPath) {
 ...
 */
function buildClassifyCode(healthJson) {
 ...
 */
function buildGlslCheckCode(targetPath) {
    const safePath = targetPath.replace(/'/g, "\\'");
    return `
import json
results = []
 ...
 */
function buildPerfBudgetCode(targetPath, budgets) {
    const safePath = targetPath.replace(/'/g, "\\'");
 ...
 */
function buildStructuredReportCode(healthJson) {
    const escapedJson = healthJson
        .replace(/\\/g, "\\\\")
        .replace(/'/g, "\\'")
        .replace(/\$/g, "\\$");
 ...
 */
function buildPopPerfCode(targetPath) {
    const safePath = targetPath.replace(/'/g, "\\'");
    return `
import json
...
// ─── Tool Registration ───────────────────────────────────────────────────────
export function registerValidateTools(server, client) {
    // ---------------------------------------------------------------------------
    // td_validate
    // ---------------------------------------------------------------------------
    server.registerTool("td_validate", {
        title: "Validate & Auto-Fix",
        description: "Validate a TouchDesigner operator/network: force-cook, detect errors, " +
        ...
        try {
            ...
            const hasErrors = health && !health.ok && health.issueCount > 0;
            ...
            if ((auto_fix ?? true) && hasErrors) {
                ...
                try {
                    result.errors_fixed = JSON.parse(fixResult.stdout || "{}");
                }
                catch {
            ...
            const glslCheck = await client.execute(buildGlslCheckCode(opPath), "/");
            let glslResults = [];
            try {
            ...
            const glslErrors = glslResults.filter((r) => r.status === "error");
            ...
            try {
                ...
                try {
                    classifyResults = JSON.parse(classifyResult.stdout || "{}");
                }
                catch {
                    classifyResults = {};
                }
            ...
            if (hasErrors) {
                try {
                    ...
                    try {
                        structuredReports = JSON.parse(reportResult.stdout || "{}");
                    }
                    catch {
                        structuredReports = {};
                    }
    ...
    // td_perf_budget
    // ---------------------------------------------------------------------------
    server.registerTool("td_perf_budget", {
        title: "Performance Budget Check",
        description: "Check performance of operators against configurable budgets. " +
    ...
    }, async ({ path: opPath, max_cook_time_ms, max_point_count, max_resolution, max_recook_hz, }) => {
        try {
            ...
            if (perfResult.violations) {
                for (const v of perfResult.violations) {
                    if (v.metric === "cookTimeMs") {
                        suggestions.push(`⚠️ ${v.path}: cook time ${v.value}ms exceeds budget ${v.budget}ms — ` +
                            `reduce complexity, lower iteration count, or cache intermediate results`);
                    }
                    else if (v.metric === "pointCount") {
                        suggestions.push(`⚠️ ${v.path}: ${v.value.toLocaleString()} points exceeds budget ${v.budget.toLocaleString()} — ` +
                            `reduce particle count, apply thin POP, or use LOD`);
                    }
                    else if (v.metric === "resolution") {
                        suggestions.push(`⚠️ ${v.path}: resolution ${v.value}px exceeds budget ${v.budget}px — ` +
            ...
            return ok({
                ...perfResult,
                suggestions,
                overallStatus: (perfResult.violationCount ?? 0) === 0 ? "within_budget" : "budget_exceeded",
            });
        }
        catch (e) {
            return err(e);
