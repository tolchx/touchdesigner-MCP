...
export function familyFromOpType(opType) {
 ...
 */
export function statsForChannel(name, raw) {
...
export function interpretationFor(stats, family) {
    if (family !== "CHOP") {
    ...
    if (stats.length === 0)
        return "No se pudieron leer canales: revisá el path y que el CHOP tenga datos.";
    const parts = [];
    ...
    if (allNormalized) {
        ...
        const r = stats
            ...
            .join(", ");
        const s0 = stats[0];
        parts.push(`Rangos medidos: ${r}. Para llevar '${s0.name}' a 0..1 usá scale=${s0.suggested_scale?.toFixed(6) ?? "n/d"} y offset=${s0.suggested_offset?.toFixed(6) ?? "n/d"}; fijate si ese rango es estable antes de hardcodear el escalar.`);
    }
    ...
    const big = stats.filter((s) => s.range !== null &&
        (s.range > 100 || (s.max !== null && Math.abs(s.max) > 100) || (s.min !== null && Math.abs(s.min) > 100)));
    if (big.length) {
        parts.push(`Ojo con el orden de magnitud de ${big.map((b) => b.name).join(", ")}: está lejos de 0..1, así que un slider 0..1 no lo toca — o normalizás con el scale sugerido o cambiás la unidad del parámetro.`);
    }
    return parts.join(" ");
}
// -----------------------------------------------------------------------------
 ...
 */
export function parseExecJson(res, what) {
...
export function registerMeasureTools(server, client) {
    server.registerTool("td_measure", {
        title: "Measure Signal Magnitudes",
        description: "Sample the REAL magnitude of a signal BEFORE choosing any scalar (scale/gain/offset/threshold). " +
        ...
        try {
            ...
            const warnings = [];
            const detail = await runBounded("td_measure", scope, () => client.getNodeDetail(scope, false));
            const d = detail?.data ?? detail;
            if (detail?.success === false) {
                throw new Error(`No se pudo leer '${scope}': ${detail?.error ?? "sin detalle"}. Verificá el path con td_find o td_operators.`);
            }
            const opType = d?.type ?? null;
            const family = familyFromOpType(opType) ?? "UNKNOWN";
            const measurements = {};
            let stats = [];
            if (family === "CHOP") {
                const res = await runBounded("td_measure", scope, () => client.readChop(scope, channels, 0, maxSamples));
                if (res?.success === false)
                    throw new Error(`CHOP ilegible: ${res?.error ?? "sin detalle"}`);
                ...
                if (names.length === 0 && declared > 0) {
                    ...
                    throw new Error(`El CHOP declara ${declared} canal(es) y no se pudo leer ninguno (¿cambió el acceso a canales en este build?): no devuelvo estadísticas vacías disfrazadas de medición.`);
                }
                if (names.length === 0)
                    warnings.push("El CHOP no tiene canales (¿vacío o sin conexión de entrada?).");
                stats = names.map((n) => statsForChannel(n, chans[n] ?? []));
                const nonFinite = stats.reduce((a, s) => a + (s.non_finite ?? 0), 0);
                if (nonFinite > 0) {
                    warnings.push(`${nonFinite} muestra(s) NaN/Inf excluidas de las estadísticas (división por cero o señal rota): no las trates como valores válidos.`);
                }
                const zeroRange = stats.filter((s) => s.is_constant).map((s) => s.name);
                if (zeroRange.length) {
                    warnings.push(`Rango cero en ${zeroRange.join(", ")}: sin información de escala (no se calculó 1/range).`);
                }
...
try:
    ...
    print(json.dumps({'success':False,'error':str(e)}))`;
                const res = await runBounded("td_measure", scope, () => client.execute(code, scope));
                ...
                const res = await runBounded("td_measure", scope, () => client.popInspect(scope));
                if (res?.success === false)
                    throw new Error(`POP ilegible: ${res?.error ?? "sin detalle"}`);
                ...
                if (!measurements.attributes) {
                    warnings.push("Este build no expone la lista de atributos del POP (t.attribs devuelve None): el conteo de puntos es válido, los atributos por punto no están disponibles.");
                }
                const pts = info?.numPoints;
                if (typeof pts === "number") {
                    measurements.attribute_bytes_per_point =
                        Array.isArray(info?.attributes) && info.attributes.length
                            ? info.attributes.reduce((a, x) => a + (x?.size ?? 0) * 4, 0)
                            : null;
                }
                if (typeof pts === "number" && pts > 500000) {
                    warnings.push(`${pts} puntos: cualquier atributo nuevo por punto cuesta memoria; medí el costo antes de agregar campos.`);
                }
            }
            else if (family === "DAT") {
                const res = await runBounded("td_measure", scope, () => client.readDat(scope, 1, 1));
                if (res?.success === false)
                    throw new Error(`DAT ilegible: ${res?.error ?? "sin detalle"}`);
                ...
                return err(`La familia '${family}' (${opType ?? "tipo desconocido"}) no se muestrea: ` +
            ...
            if (family === "CHOP" && stats.length) {
                if (stats.every((s) => s.looks_normalized))
                    units_guess = "normalizado 0..1";
                else if (stats.every((s) => s.looks_unit))
                    units_guess = "bipolar -1..1";
                else
                    units_guess = "crudo (sin normalizar)";
            }
            else if (family === "TOP")
                units_guess = "pixeles";
            else if (family === "POP")
                units_guess = "conteo de geometria";
            else if (family === "DAT")
                units_guess = "filas de texto";
            const result = {
                path: scope,
                family,
                type: opType,
