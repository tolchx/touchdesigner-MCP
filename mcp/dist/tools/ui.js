...
export function registerUiTools(server, client) {
    ...
    server.registerTool("td_screenshot", {
        ...
        inputSchema: {
            path: z
                .string()
                .optional()
                .describe("Operator path to screenshot (default: current active pane)"),
        },
    }, async ({ path: opPath }) => {
        try {
    ...
    // td_get_screenshots
    // ---------------------------------------------------------------------------
    server.registerTool("td_get_screenshots", {
    ...
    // td_navigate_to
    // ---------------------------------------------------------------------------
    server.registerTool("td_navigate_to", {
        title: "Navigate To",
    ...
    // td_read_textport
    // ---------------------------------------------------------------------------
    server.registerTool("td_read_textport", {
        title: "Read Textport",
    ...
    // td_clear_textport
    // ---------------------------------------------------------------------------
    server.registerTool("td_clear_textport", {
    ...
    server.registerTool("td_search", {
        ...
        try {
            ...
            const result = await runBounded("td_search", rootScope, () => client.searchInTD(query, rootScope, scope, case_sensitive, boundedMax, count_only));
            return ok(result);
        }
        catch (e) {
            return err(e);
    ...
    // td_reinit_extension
    // ---------------------------------------------------------------------------
    server.registerTool("td_reinit_extension", {
    ...
    // td_auto_layout
    // ---------------------------------------------------------------------------
    server.registerTool("td_auto_layout", {
        title: "Auto-Layout Network",
        description: "Auto-arrange operators in a container using topological-sort layout. Sources on the left, outputs on the right, organized as a clean grid. Supports all operator families including POPs.",
        inputSchema: {
            path: z
                .string()
                .default("/project1")
        ...
        try {
            const result = await client.autoLayout(path ?? "/project1", spacingX ?? 250, spacingY ?? 80);
            return ok(result);
        }
        catch (e) {
            return err(e);
    ...
    // td_smart_connect
    // ---------------------------------------------------------------------------
    server.registerTool("td_smart_connect", {
        title: "Smart Connect",
        description: "Create an operator between two existing operators with auto-detected type compatibility. Auto-positions and wires the new operator. Either source or destination may be omitted, but at least one is required.",
        inputSchema: {
            source: z
                .string()
            ...
            type: z
                .string()
                .optional()
                .describe("Optional forced operator type (e.g. 'blurTOP', 'nullCHOP'). If omitted, a compatible null* type is auto-detected from the source/destination family."),
            name: z
                .string()
                .optional()
