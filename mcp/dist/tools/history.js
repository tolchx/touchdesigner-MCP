 ...
 */
export async function recordChange(client, path, action) {
    try {
...
export function registerHistoryTools(server, client) {
    ...
    // td_history_list
    // ---------------------------------------------------------------------------
    server.registerTool("td_history_list", {
        title: "List Change History",
        description: "List the last N recorded changes (history entries) stored in memory. Each entry shows the operator path, action, and timestamp. Does NOT call into TouchDesigner.",
        inputSchema: {
            limit: z
                .number()
                .int()
    ...
    // td_history_undo
    // ---------------------------------------------------------------------------
    server.registerTool("td_history_undo", {
        title: "Undo Last Change",
        description: "Revert the most recent recorded change by restoring the operator's parameters from its pre-change snapshot. Only affects in-memory history; uses snapshot_scene + setParameters.",
        inputSchema: {},
    }, async () => {
        try {
            if (history.length === 0) {
            ...
            async function restoreNode(node) {
                if (!node || !node.path || !node.pars)
                    return;
                const pars = node.pars;
                const updates = [];
    ...
    // td_history_clear
    // ---------------------------------------------------------------------------
    server.registerTool("td_history_clear", {
        title: "Clear Change History",
        description: "Clear all in-memory history entries. This does NOT affect TouchDesigner — only discards the local undo log.",
        inputSchema: {},
    }, async () => {
        try {
