...
export function registerKnowledgeTools(server, client) {
    ...
    // td_pops_query
    // ---------------------------------------------------------------------------
    server.registerTool("td_pops_query", {
        title: "Query POPs Knowledge Base",
        description: "Search the local POPs knowledge base or fetch a specific operator doc by page slug (e.g. Particle_POP).",
        inputSchema: {
    ...
    // td_ops_query
    // ---------------------------------------------------------------------------
    server.registerTool("td_ops_query", {
        title: "Query Operator Knowledge Base (TOP/CHOP/SOP/DAT)",
        description: "Search the local operator knowledge base (TOP/CHOP/SOP/DAT) or fetch a specific operator doc by family+page slug (e.g. TOP + Noise_TOP).",
        inputSchema: {
            search: z.string().optional().describe("Search string"),
    ...
    // td_templates_query
    // ---------------------------------------------------------------------------
    server.registerTool("td_templates_query", {
        title: "Query Project Templates",
        description: "Search reusable patterns and project-specific markdown templates inside Toe_Expand documentation.",
        inputSchema: {
    ...
    // td_alias_resolve
    // ---------------------------------------------------------------------------
    server.registerTool("td_alias_resolve", {
        title: "Resolve Semantic Aliases",
        description: "Resolve prompt vocabulary like feedback loop, life, size, cd or direction into canonical TouchDesigner parameters, attributes and family hints.",
        inputSchema: {
    ...
    // td_get_param_help
    // ---------------------------------------------------------------------------
    server.registerTool("td_get_param_help", {
        title: "Get Parameter Help",
        description: "Look up the available parameters for a TouchDesigner operator type (e.g. 'noiseTOP') from the local knowledge base.",
        inputSchema: {
            type: z
                .string()
                .describe("Operator type to look up (e.g. 'noiseTOP', 'constantCHOP', 'mergeSOP', 'textDAT')"),
        },
    }, async ({ type }) => {
        try {
            const opType = type;
            ...
            const matching = ops.filter((op) => op.tdOpTypeGuess?.toLowerCase() === opType.toLowerCase()
            ...
            try {
                ...
                const popsMatches = pops.filter((op) => op.tdOpTypeGuess?.toLowerCase() === opType.toLowerCase()
                    || op.pageSlug?.toLowerCase() === opType.toLowerCase());
                ...
                const popCandidates = ops.filter((op) => op.pageSlug?.toUpperCase().includes("POP") &&
            ...
            if (isPop) {
                ...
                const docPath = path.join(__dirname, "../data/pops/operators", `${popMatch.pageSlug}.json`);
                let doc;
                try {
            ...
            const __dirname = path.dirname(fileURLToPath(import.meta.url));
            const docPath = path.join(__dirname, "../data/ops/operators", match.family, `${match.pageSlug}.json`);
            let doc;
            try {
                const raw = await fs.readFile(docPath, "utf8");
                doc = JSON.parse(raw);
    ...
    // ---------------------------------------------------------------------------
    server.registerTool("td_search_official_docs", {
    ...
    // td_export_network
    // ---------------------------------------------------------------------------
    server.registerTool("td_export_network", {
        title: "Export Network as Python Code, Diff, or JSON",
        description: "Generate Python code, a simplified git-diff, or a JSON snapshot that captures the structure of a TouchDesigner network. Introspects the children of a container operator and produces output in the requested format.",
        inputSchema: {
            path: z
                .string()
...
try:
    ...
    print(json.dumps({'success': False, 'error': str(e)}))`;
            ...
            const allNodes = [];
            function collectNodes(items, parentPath) {
            ...
            for (const node of allNodes) {
                if (node.pars && node.pars.length > 0) {
                    const nonDefaultPars = node.pars.filter((p) => JSON.stringify(p.val) !== JSON.stringify(p.default));
    ...
    // td_list_tutorials
    // ---------------------------------------------------------------------------
    server.registerTool("td_list_tutorials", {
        title: "List Tutorials",
        description: "List available TouchDesigner tutorials. Filter by category (audio, glow, feedback, particles, shader) and/or difficulty (beginner, intermediate, advanced).",
        inputSchema: {
    ...
    // td_get_tutorial
    // ---------------------------------------------------------------------------
    server.registerTool("td_get_tutorial", {
        title: "Get Tutorial Content",
        description: "Get the full markdown content of a tutorial by its name (filename without .md extension). Use td_list_tutorials to find available tutorials.",
        inputSchema: {
    ...
    // td_list_workflows
    // ---------------------------------------------------------------------------
    server.registerTool("td_list_workflows", {
        title: "List Workflows",
        description: "List available reusable TouchDesigner workflow patterns. Filter by category (color, blur, keying, feedback, audio, transform, analyze) and/or difficulty (beginner, intermediate, advanced).",
        inputSchema: {
    ...
    // td_get_workflow
    // ---------------------------------------------------------------------------
    server.registerTool("td_get_workflow", {
        title: "Get Workflow Content",
        description: "Get the full markdown content of a workflow by its name (filename without .md extension). Use td_list_workflows to find available workflows.",
        inputSchema: {
    ...
    // td_get_td_classes
    // ---------------------------------------------------------------------------
    server.registerTool("td_get_td_classes", {
        title: "List TD Operator Classes",
        description: "List all TouchDesigner operator classes from the local knowledge base, organized by family (TOP, CHOP, SOP, DAT, POP, COMP). Each class includes its page title, slug, URL, and optional tdOpTypeGuess.",
        inputSchema: {
        ...
        try {
            ...
            // Add POP family
            families["POP"] = (popsIndex.operators || []).map((op) => ({
                pageTitle: op.pageTitle,
                pageSlug: op.pageSlug,
                tdOpTypeGuess: op.tdOpTypeGuess,
                url: op.url,
            ...
            for (const fam of familiesToInclude) {
                ...
                if (search) {
                    ...
                    ops = ops.filter((op) => (op.pageTitle?.toLowerCase() || "").includes(q) ||
    ...
    // td_get_module_help
    // ---------------------------------------------------------------------------
    server.registerTool("td_get_module_help", {
        title: "Get Operator Documentation",
        description: "Look up detailed documentation for a specific TouchDesigner operator by its page slug or tdOpTypeGuess. Searches both the main operator index (TOP/CHOP/SOP/DAT) and POPs index (POP). Returns parameter details, inputs, attributes, and usage examples.",
        inputSchema: {
    ...
    // ---------------------------------------------------------------------------
    server.registerTool("td_compare_mcps", {
        title: "Compare MCP Servers (vs TWOZERO & co.)",
        description: "Compare this TouchDesigner MCP against other existing MCPs and tools " +
        ...
        try {
            ...
            return ok({
                ...
                comparison: filtered.map((s) => ({
                    server: s.server,
                    ...Object.fromEntries(categories.map((c) => [c.label, s[c.key]])),
                    notes: s.notes,
                })),
                table,
    ...
    // ---------------------------------------------------------------------------
    server.registerTool("td_run_prompt", {
        title: "Run Master Prompt",
        description: "Retrieve a master prompt by its ID. Master prompts are Markdown files " +
        ...
        inputSchema: {
            prompt_id: z
                .string()
                .describe("Prompt ID (matches a .md file inside prompts/master/, e.g. 'particles-experto-feedback-sim')"),
            params: z
                .record(z.string(), z.string())
                .optional()
                .describe("Optional key-value pairs for {{variable}} substitution in the prompt content"),
        },
        ...
        try {
            ...
            if (params && typeof params === "object") {
                for (const [key, value] of Object.entries(params)) {
                    renderedBody = renderedBody.replace(new RegExp(`\\{\\{\\s*${key}\\s*\\}\\}`, "g"), value);
                }
            }
