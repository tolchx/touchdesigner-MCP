 ...
 */
function startSession(client, path, intervalSec, thresholdMs, maxSamples) {
    // Stop existing session
    stopSession();
    const intervalMs = Math.max(1000, Math.round(intervalSec * 1000));
    const history = new Array(maxSamples);
    session = {
        active: true,
        path,
        interval: intervalMs,
 ...
 */
async function takeSample(client) {
    if (!session)
        return;
    try {
        const result = await client.getPerf(session.path, 20);
...
// ── Registered tool ────────────────────────────────────────────────────
export function registerWatchdogTools(server, client) {
    // ---------------------------------------------------------------------------
    // td_watch — real-time performance monitoring
    // ---------------------------------------------------------------------------
    server.registerTool("td_watch", {
        title: "Watch Performance",
        description: "Start or stop real-time performance monitoring in TouchDesigner. " +
        ...
        inputSchema: {
            ...
            action: z
                ...
                .optional()
                .default("snapshot")
                .describe("'start' begins monitoring, 'stop' ends it, 'snapshot' returns current state (default: snapshot)"),
        },
