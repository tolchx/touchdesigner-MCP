...
export function registerCrudTools(server, client) {
    ...
    // td_create_operator
    // ---------------------------------------------------------------------------
    server.registerTool("td_create_operator", {
        title: "Create Operator",
        description: "Create a new operator in TouchDesigner. Specify the opType (e.g. noiseTOP, constantTOP, nullCHOP) and optional name, parent path, and position. For GLSL POPs (glslPOP, glslCreatePOP, etc.), outputattrs is auto-set to 'P' and numelems to 100 unless overridden — these are required for P[id]/Cd[id] writes to compile.",
        inputSchema: {
            type: z
                .string()
                .describe("The TD opType to create (e.g. 'noiseTOP', 'constantCHOP', 'nullSOP')"),
                ...
                if (createdPath) {
                    ...
                    if (updates.length > 0) {
                        try {
                            ...
                            if (outputattrs !== "")
                                resultExt.outputattrs = outputattrs ?? "P";
                            if (numelems !== 0)
                                resultExt.numelems = numelems ?? 100;
                        }
                        catch {
    ...
    // td_delete_operator
    // ---------------------------------------------------------------------------
    server.registerTool("td_delete_operator", {
    ...
    // td_connect_nodes
    // ---------------------------------------------------------------------------
    server.registerTool("td_connect_nodes", {
        title: "Connect Nodes",
    ...
    // td_copy_node
    // ---------------------------------------------------------------------------
    server.registerTool("td_copy_node", {
        title: "Copy Node",
