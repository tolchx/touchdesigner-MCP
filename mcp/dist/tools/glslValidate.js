...
export function analyzeGlslShader(code) {
    ...
    if (TD_NUM_ELEMENTS_WITH_ARG_RE.test(normalized)) {
        errors.push("TDNumElements() NO toma índice de input (es la cantidad de hilos pedidos): " +
    ...
    if (multiInputs.size > 0) {
        warnings.push("lee de input(s) " + [...multiInputs].join(", ") +
 ...
 */
export function analyzeGlslTopShader(code, creationCode) {
    const normalized = code.replace(/\r\n/g, "\n");
    const errors = [];
    const warnings = [];
    const has_fragcolor_out = /\bout\s+vec4\s+fragColor\b/.test(normalized) ||
        /layout\s*\([^)]*location\s*=\s*0[^)]*\)\s*out\s+vec4\s+fragColor\b/.test(normalized);
    if (!has_fragcolor_out) {
        errors.push("Regla TOP 1 (GLSL_TOP_RULES.md): falta 'out vec4 fragColor;' — el pixel shader " +
            "debe declarar su salida: layout(location = 0) out vec4 fragColor;");
    }
    const bad_uv_swizzles = [...new Set([...normalized.matchAll(BAD_UV_RE)].map((m) => "vUV." + m[1]))];
    if (bad_uv_swizzles.length > 0) {
        errors.push("Regla TOP 2 (GLSL_TOP_RULES.md): " + bad_uv_swizzles.join(", ") +
    ...
    const uses_uniform0name_risk = !!creationCode && /uniform\d+name/.test(creationCode) &&
        !/vec\d+name|const\d+name|matrix\d+name|ac\d+name/.test(creationCode);
    if (uses_uniform0name_risk) {
        errors.push("Regla TOP 4 (GLSL_TOP_RULES.md, probe D): en glslTOP NO existe uniform0name " +
    ...
    if (/texture\s*\(\s*sTD2DInputs\[\d+\]\s*,\s*vUV\./.test(normalized)) {
        warnings.push("lee sTD2DInputs con vUV — en redes de feedback el buffer no avanza con " +
...
export function preValidateTopShader(code, creationCode) {
