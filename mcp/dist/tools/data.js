...
export function registerDataTools(server, client) {
    ...
    // td_read_dat
    // ---------------------------------------------------------------------------
    server.registerTool("td_read_dat", {
        title: "Read DAT",
    ...
    // td_write_dat
    // ---------------------------------------------------------------------------
    server.registerTool("td_write_dat", {
        title: "Write DAT",
        ...
        try {
            const result = await client.writeDat(opPath, text, old_text, new_text, replace_all ?? false);
            return ok(result);
        }
        catch (e) {
            return err(e);
    ...
    // td_read_chop
    // ---------------------------------------------------------------------------
    server.registerTool("td_read_chop", {
        title: "Read CHOP",
