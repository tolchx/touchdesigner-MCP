    ...
    particlePOP: [
        {
            ...
            severity: "error",
            description: "particlePOP REQUIRES a Null POP as feedback target via the particlesupdatepop parameter",
    ...
    feedbackPOP: [
        {
            ...
            severity: "error",
            description: "feedbackPOP needs its 'target' parameter set to the downstream nullPOP",
        ...
        },
        {
            ...
            severity: "warning",
            description: "feedbackPOP should NOT connect directly back to itself — use a nullPOP as intermediate",
    ...
    glslPOP: [
        {
            ...
            severity: "warning",
            description: "glslPOP requires matching attribute declarations (P, Vel, Cd) in the GLSL shader",
    ...
    glslCreatePOP: [
        {
            name: "attribute_alignment",
            severity: "warning",
            description: "glslCreatePOP generates points — ensure shader outputs match expected POP attributes",
    ...
    neighborPOP: [
        {
            ...
            severity: "warning",
            description: "neighborPOP is O(n²) — performance degrades rapidly above 100K points",
    ...
    copyPOP: [
        {
            name: "two_inputs",
            severity: "error",
            description: "copyPOP needs two inputs: source geometry (input 0) and template geometry (input 1)",
    ...
    blendPOP: [
        {
            name: "two_inputs",
            severity: "error",
            description: "blendPOP needs two inputs to blend: primary chain (input 0) and secondary chain (input 1)",
    ...
    fieldPOP: [
        {
            ...
            severity: "warning",
            description: "fieldPOP creates proximity fields — must be placed near attractor/repulsor geometry",
    ...
    deletePOP: [
        {
            ...
            severity: "warning",
            description: "deletePOP removes points — ensure a delete condition is set, otherwise it deletes ALL points",
    ...
    cachePOP: [
        {
            name: "cache_size",
            severity: "warning",
            description: "cachePOP stores frames in memory — large caches (>256 frames) consume significant VRAM",
            fix: "Reduce cache length or resolution for large point counts",
        },
    ],
};
// ─── Cross-Family Connection Rules ────────────────────────────────────────
export const INVALID_CROSS_FAMILY = {
 ...
 */
export function buildPopScanCode(rootPath) {
 ...
 */
export function buildCrossFamilyCheckCode(rootPath) {
    const safePath = rootPath.replace(/'/g, "\\'");
    return `
import json
...
// ─── Tool Registration ───────────────────────────────────────────────────
export function registerPopValidationTools(server, client) {
    // ── td_pop_validate ──────────────────────────────────────────────────────
    server.registerTool("td_pop_validate", {
        title: "POP Network Validation",
        description: "Validate a POP (Particle Operator) network for common errors: " +
            ...
            for (const pop of scanData.pops || []) {
                ...
                for (const rule of rules) {
                    ...
                    switch (rule.name) {
                        ...
                        case "not_self_connecting":
                            if (opType === "feedbackPOP" && pop.hasInput) {
                                const selfRef = pop.inputSources.some((s) => s.path === pop.path);
                                if (selfRef) {
                                    violated = true;
                    ...
                    if (violated) {
                        ...
                        result.fixSuggestions.push(`[${rule.severity.toUpperCase()}] ${pop.path} (${opType}): ${rule.fix}`);
            ...
            if (check_cross_family) {
                ...
                for (const v of cfData.violations || []) {
                    if (v.isInvalid) {
                        ...
                        result.fixSuggestions.push(`[ERROR] ${v.source} (${v.sourceFamily}) → ${v.target} (${v.targetFamily}): use ${bridge}`);
                        result.ok = false;
                    }
                }
            }
...
`;
                    ...
                    for (const r of attrData) {
                        if (!r.present) {
                            ...
                            result.fixSuggestions.push(`[WARNING] ${r.path}: missing attribute '${r.attribute}' — add attributePOP to initialize`);
                        }
                    }
                }
            }
