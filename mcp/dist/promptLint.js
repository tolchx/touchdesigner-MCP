...
async function main() {
    ...
    result.operatorSuggestions = semantic.operatorHints.map((h) => `${h.canonical} (${h.family ?? "?"})`);
    ...
    if (prompt.toLowerCase().includes("particle") &&
    ...
    const mixedFamilies = semantic.familyHints.includes("POP") &&
