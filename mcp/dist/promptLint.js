...
// mcp/src/semantic.ts
var SemanticResolutionSchema = external_exports.object({
  original: external_exports.string(),
  normalized: external_exports.string(),
  conceptMatches: external_exports.array(
    external_exports.object({
      concept: external_exports.string(),
      canonical: external_exports.string(),
      aliases: external_exports.array(external_exports.string()),
      note: external_exports.string().optional()
    })
  ),
  familyHints: external_exports.array(external_exports.string()),
  parameterHints: external_exports.array(
    external_exports.object({
  ...
  attributeHints: external_exports.array(
    external_exports.object({
      requested: external_exports.string(),
      canonical: external_exports.string(),
      note: external_exports.string().optional()
  ...
  operatorHints: external_exports.array(
    external_exports.object({
      requested: external_exports.string(),
      canonical: external_exports.string(),
...
var CONCEPTS = [
  {
    ...
    note: "En sistemas POP de part\xEDculas suele resolverse al par\xE1metro que apunta al final del solver."
  },
  {
  ...
  },
  {
    ...
    aliases: ["point scale", "point size", "size", "tama\xF1o", "escala de punto"]
  },
  {
  ...
  },
  {
    ...
    note: "Ruido libre de divergencia para simulaci\xF3n de fluidos en Noise POP (modo curl)."
  },
  {
  ...
  },
  {
    concept: "SPH fluid",
    canonical: "SPH",
    aliases: ["sph", "fluid", "fluido", "smoothed particle hydrodynamics", "hidrodin\xE1mica"],
    note: "Simulaci\xF3n de fluidos con Smoothed Particle Hydrodynamics v\xEDa GLSL POP compute."
  },
  {
    concept: "instancing",
    canonical: "Instancing",
    aliases: ["instancing", "instance", "instancia", "copias", "copy sop", "geometry comp", "copy pop"],
    note: "Instanciado de geometr\xEDa usando geometryCOMP + copySOP o Copy POP."
  },
  {
  ...
  },
  {
    ...
    canonical: "DMX",
    aliases: ["dmx", "lighting", "iluminaci\xF3n", "art-net", "artnet", "sacn", "sACN", "dmx fixture"],
    note: "Control de iluminaci\xF3n DMX mediante DMX Out CHOP / DMX Fixture POP."
  },
  {
  ...
  },
  {
    ...
    canonical: "Cache",
    aliases: ["cache", "caching", "playback", "reproducci\xF3n", "buffer", "cache pop", "cache select pop"]
  },
  {
    concept: "neighbor detection",
    canonical: "Neighbor",
    aliases: ["neighbor", "neighbour", "vecino", "proximity", "cerca", "interaction", "neighbor pop"],
    note: "Detecci\xF3n de puntos vecinos para interacciones POP mediante Neighbor POP."
  },
  {
    concept: "atomic operations",
    canonical: "Atomic",
    aliases: ["atomic", "collision", "colisi\xF3n", "buffer atomico", "ssbo", "glsl atomic"],
    note: "Operaciones at\xF3micas en GLSL POP para colisiones y buffers compartidos."
  },
  {
  ...
  },
  {
    ...
    canonical: "PreRoll",
    aliases: ["pre-roll", "preroll", "pre roll", "initial simulation", "simulaci\xF3n inicial", "steady state"],
    note: "Pre-Roll en Particle POP para simular frames antes del inicio."
  },
  {
    concept: "particle emission",
    canonical: "ParticleEmission",
    aliases: ["emission", "emisi\xF3n", "birth rate", "tasa de nacimiento", "rate", "emit"],
    note: "Control de emisi\xF3n en Particle POP: Rate (velocidad) + Life (vida m\xE1xima)."
  },
  {
    concept: "field weight",
  ...
  },
  {
    ...
    note: "Geometr\xEDas disponibles en Field POP: Sphere, Box, Tube, Capsule."
  },
  {
    concept: "copy pop instancing",
    canonical: "CopyPopInstancing",
    aliases: ["copy pop", "copy pop instancing", "multiplicar geometr\xEDa", "multiply geometry", "standalone mode", "template mode"],
...
var PARAMETER_ALIASES = [
  ...
  { requested: "size", canonical: "pointscale", note: "Alias t\xEDpico en POPs." },
  { requested: "feedback target", canonical: "particlesupdatepop", note: "Par\xE1metro del solver POP." },
  { requested: "radius", canonical: "radx/rady/radz o radius", note: "Var\xEDa entre operadores." },
  { requested: "res", canonical: "resolution", note: "Com\xFAn en TOPs." },
  { requested: "birth rate", canonical: "birthrate", note: "Tasa de nacimiento en Particle POP." },
  { requested: "drag", canonical: "drag", note: "Amortiguaci\xF3n/aire en forces." },
  ...
  { requested: "damping", canonical: "drag", note: "Amortiguaci\xF3n en Particle POP." },
  ...
  { requested: "partvel", canonical: "PartVel", note: "Velocidad de part\xEDcula." },
  { requested: "partlife", canonical: "PartLife", note: "Vida de part\xEDcula." },
...
var ATTRIBUTE_ALIASES = [
  { requested: "p", canonical: "P / _P", note: "Posici\xF3n." },
  ...
  { requested: "age", canonical: "age / _AGE", note: "Edad de part\xEDcula." },
  { requested: "life", canonical: "life / _LIFE", note: "Vida m\xE1xima." },
  ...
  { requested: "rot", canonical: "Rot / _ROT", note: "Rotaci\xF3n." },
  { requested: "pscale", canonical: "pointscale", note: "Escala de punto." },
  { requested: "width", canonical: "LineWidth", note: "Ancho de l\xEDnea." },
...
function resolveSemanticTerms(input) {
  const normalized = input.toLowerCase().trim();
  const conceptMatches = CONCEPTS.filter(
    (item) => item.aliases.some((alias) => containsAlias(normalized, alias))
  );
  const familyHints = FAMILY_HINTS.filter(
    (item) => item.aliases.some((alias) => containsAlias(normalized, alias))
  ).map((item) => item.family);
  const parameterHints = PARAMETER_ALIASES.filter(
    (item) => containsAlias(normalized, item.requested)
  );
  const attributeHints = ATTRIBUTE_ALIASES.filter(
    (item) => containsAlias(normalized, item.requested)
  );
  const operatorHints = OPERATOR_HINTS.filter(
    (item) => containsAlias(normalized, item.requested)
  );
...
async function main() {
  ...
  if (semantic.operatorHints.length === 0) {
    result.issues.push({
      severity: "warning",
      message: "No se detectaron operadores espec\xEDficos en el prompt. Considera usar t\xE9rminos m\xE1s concretos."
    });
  }
  if (semantic.familyHints.length === 0) {
    result.issues.push({
      severity: "info",
      message: "No se detect\xF3 una familia espec\xEDfica (POP/TOP/CHOP/SOP/DAT). El planificador usar\xE1 detecci\xF3n autom\xE1tica."
    });
  }
  if (prompt.toLowerCase().includes("particle") && !prompt.toLowerCase().includes("feedback")) {
    result.issues.push({
      severity: "info",
      message: "Sistema de part\xEDculas sin Feedback POP puede quedar funcional pero menos \xFAtil para simulaciones temporales."
    });
  }
  const mixedFamilies = semantic.familyHints.includes("POP") && semantic.familyHints.includes("TOP") && !prompt.toLowerCase().includes("to pop") && !prompt.toLowerCase().includes("bridge");
  if (mixedFamilies) {
    result.issues.push({
      severity: "warning",
      message: "Se mezclan familias POP y TOP sin bridge expl\xEDcito (TOP to POP). Las conexiones no funcionar\xE1n entre familias."
    });
  }
