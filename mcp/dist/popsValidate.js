 ...
 */
import { getPopInfo, suggestParameterNames, formatUnknownParameterError, } from "./popKnowledge.js";
export function validatePopParameters(opType, requestedNames) {
...
export function assertValidPopParameters(opType, requestedNames) {
    const v = validatePopParameters(opType, requestedNames);
    if (!v.ok) {
        throw new Error(formatUnknownParameterError(opType, v.unknown, v.knownParamCount));
    }
    return v;
}
/**
 ...
 */
export async function validateParameterNamesForPath(client, path, names, opTypeHint) {
    ...
    try {
        const result = await client.getParameters(path);
        const liveNames = (result?.parameters ?? [])
            .map((p) => p?.name)
            .filter(Boolean);
        if (liveNames.length > 0) {
            ...
            return {
                ...
                detail: unknown.length === 0
                    ...
                    : formatUnknownParameterError(opTypeHint ?? path, unknown, liveNames.length),
            };
        }
    }
    catch {
    ...
    if (!kb.skipped) {
        return {
            ...
            detail: kb.ok
                ...
                : formatUnknownParameterError(opTypeHint ?? path, kb.unknown, kb.knownParamCount),
        };
    }
 ...
 */
export const POP_NETWORK_TEMPLATES = [
    {
        name: "pop-line-merge-copy",
        description: "Two line strips merged and copied along template points — the single most common POP chain in real projects.",
...
build_pop_line_merge_copy('/project1')`,
    },
    {
        ...
        description: "Two circles merged into a switcher for A/B source selection — second most common corpus chain.",
...
build_pop_circle_merge_switch('/project1')`,
    },
    {
        ...
        description: "Grid of points plus an attribute POP that adds an instancing attribute (scale/pivot per point) — the standard instancing setup.",
...
build_pop_grid_attribute('/project1')`,
    },
    {
        ...
        description: "Sphere animated by a transform and recorded into a trail — classic motion-graphics setup.",
...
build_pop_sphere_transform_trail('/project1')`,
    },
    {
        ...
        description: "boxPOP fed into two chained GLSL POPs (glsl→glsl: 140 corpus occurrences — the dominant compute pattern).",
...
build_pop_box_glsl_chain('/project1')`,
    },
    {
        ...
        description: "sprinklePOP scatters points on a sphere surface, particlePOP simulates them, and the result loops back through the particle system (corpus: sprinkle→particle x4; particle systems update from a feedback POP).",
 ...
 */
export function searchPopTemplates(query) {
    const q = query.toLowerCase().trim();
    if (!q)
        return [...POP_NETWORK_TEMPLATES];
    return POP_NETWORK_TEMPLATES.filter((t) => t.name.toLowerCase().includes(q) ||
        t.description.toLowerCase().includes(q) ||
        t.corpusChain.toLowerCase().includes(q));
}
/** Get a POP template by exact name. */
export function getPopTemplateByName(name) {
 ...
 */
export function popTemplatesAsNetworkTemplates() {
 ...
 */
export async function resolvePopTypeForPath(client, path) {
    try {
        const detail = await client.getNodeDetail(path);
        const t = detail?.data?.type ?? detail?.type;
        if (typeof t === "string" && getPopInfo(t))
            return t;
        return null;
    }
    catch {
