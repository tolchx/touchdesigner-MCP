 ...
 */
export function detectSlotCollisions(graph) {
    const warnings = [];
        ...
        for (const [slot, slotConns] of bySlot) {
            if (slotConns.length > 1) {
                const edges = slotConns
                    ...
                    .join(", ");
                warnings.push(`Slot collision on ${destId} input ${slot}: ${edges}. ` +
        ...
        try {
            ...
            if (node.parameters && Object.keys(node.parameters).length > 0) {
                try {
                    ...
                    if (!pv.ok) {
                        errors.push(`Params for ${node.id} (${node.opType}): ${pv.unknown
                            .map((u) => `'${u.name}'` +
