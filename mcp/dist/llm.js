...
export function createLlmClientFromEnv() {
    ...
    if (provider === "ollama") {
        ...
        const modelsFromList = process.env.OLLAMA_MODELS?.split(",")
            .map((m) => m.trim())
            .filter(Boolean) ?? [];
        const model = explicitModel && explicitModel.length > 0
            ? explicitModel
            : "gemma:2b";
    ...
    if (provider === "gemini") {
        ...
        const modelsFromList = process.env.GEMINI_MODELS?.split(",")
            .map((m) => m.trim())
            .filter(Boolean) ?? [];
