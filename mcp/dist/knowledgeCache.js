...
function detectFamilyFromKey(key) {
 ...
 */
export function ensureKnowledgeLoaded() {
    if (_loaded)
        return;
    try {
        ...
        _loadError = e;
        console.warn("[knowledgeCache] Failed to load knowledge base:", e.message || String(e));
 ...
 */
export function buildRawMap(parsed, family, dataDir) {
    const map = {};
    if (Array.isArray(parsed)) {
        ...
        for (const [name, path] of Object.entries(parsed)) {
            try {
                const opPath = resolve(dataDir, family, path);
...
export function getKnowledgeLoadError() {
    return _loadError;
}
// ─── Search index builder ───────────────────────────────────────────────────
export function buildSearchIndex(opsMap, popsMap) {
    const index = new Map();
    ...
    for (const map of allMaps) {
        for (const [key, op] of Object.entries(map)) {
            ...
            if (op.name)
                index.set(op.name.toLowerCase(), entry);
        }
    }
    return index;
}
...
export async function loadOpsOperatorDoc(family, pageSlug) {
    ...
    const dataDir = getDataDir();
    const raw = await readFile(resolve(dataDir, `ops/operators/${family}/${safe}.json`), "utf8");
...
export async function loadPopsOperatorDoc(pageSlug) {
    const safe = pageSlug.replaceAll("..", "").replaceAll("\\", "/");
    const dataDir = getDataDir();
    const raw = await readFile(resolve(dataDir, `pops/operators/${safe}.json`), "utf8");
