...
function formatTopologyForPrompt(catalog) {
    const lines = [];
    const byFamily = new Map();
    catalog.forEach((topo) => {
        const existing = byFamily.get(topo.family) || [];
        existing.push(topo);
        byFamily.set(topo.family, existing);
    });
    byFamily.forEach((ops, family) => {
        lines.push(`\n## ${family} Operators`);
 ...
 */
export async function llmPlanNetwork(prompt, catalog, targetPath) {
...
Generate the network graph JSON.`;
    try {
        ...
        const nodes = (llmGraph.nodes || []).map((n, i) => ({
            id: n.id || `n${i}`,
        ...
        const connections = (llmGraph.connections || []).map((c) => ({
            from: c.from,
            to: c.to,
