 ...
 */
function formatListByType(items, options, config) {
    ...
    const fields = detailLevel === "minimal"
    ...
    switch (responseFormat) {
        ...
        case "markdown":
            return formatAsMarkdownTable(rows, fields);
        case "text":
            if (config.headingFn) {
                return formatAsBulletList(rows, fields, (item) => config.headingFn(item, detailLevel));
            }
            return formatAsBulletList(rows, fields, (item) => String(item.name ?? "?"));
        default:
            return JSON.stringify(rows, null, 2);
    }
}
...
const OPERATOR_SHAPE = {
    ...
    headingFn: (item) => item.name ? `${item.name} (${item.path ?? "?"})` : String(item.path ?? "?"),
};
const PARAMETER_SHAPE = {
    minimalFields: ["name"],
    ...
    headingFn: (item) => {
        ...
        if (item.val !== undefined)
            parts.push(`= ${item.val}`);
        if (item.expr)
            parts.push(`[expr: ${item.expr}]`);
        return parts.join(" ");
    },
};
const CONNECTION_SHAPE = {
 ...
 */
export function formatResponse(data, options, shape) {
    ...
    switch (shape) {
        case "operatorList":
            return formatListByType(data, options, OPERATOR_SHAPE);
        case "parameterList":
            return formatListByType(data, options, PARAMETER_SHAPE);
        case "errorList":
            return formatErrorList(data, options);
        case "connectionList":
            return formatListByType(data, options, CONNECTION_SHAPE);
        case "graph":
            return formatNetworkGraph(data, options);
        default:
            return formatGeneric(data, options);
    }
}
// ─── Generic fallback formatter ─────────────────────────────────────────────
function formatGeneric(data, options) {
    const { responseFormat } = options;
    switch (responseFormat) {
        case "json":
 ...
 */
export function formatErrorList(errors, options) {
    const { detailLevel, responseFormat } = options;
...
export function formatConnectionList(connections, options) {
 ...
 */
export function formatNetworkGraph(graph, options) {
    const { detailLevel, responseFormat } = options;
...
function formatAsMarkdownTable(rows, columns) {
    if (rows.length === 0)
        return "*(empty)*";
    // Build header
...
function formatMarkdownCell(value) {
    ...
    if (typeof value === "object")
        return "…"; // collapse nested objects
    const s = String(value);
    // Escape pipe characters in cell content
    return s.replace(/\|/g, "\\|").replace(/\n/g, " ");
}
// ─── Utility: Bullet-list builder ───────────────────────────────────────────
function formatAsBulletList(items, fields, headingFn) {
    if (items.length === 0)
        return "(none)";
    const lines = [];
...
export function minimalJson(data, shape) {
...
export function summaryMarkdown(data, shape) {
