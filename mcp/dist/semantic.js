...
function wordCount(normalizedPrompt, word) {
 ...
 */
export function resolveOperatorType(prompt) {
 ...
 */
export function resolveAllOperatorTypes(prompt, topN = 5) {
    const norm = normalizePrompt(prompt);
    if (!norm)
        return [];
    const scores = [];
 ...
 */
export function getBestFamily(prompt) {
    const norm = normalizePrompt(prompt);
    if (!norm)
        return "unknown";
    const scores = [];
 ...
 */
export function getAllFamilies(prompt) {
    const norm = normalizePrompt(prompt);
    if (!norm)
        return [];
    const scores = [];
