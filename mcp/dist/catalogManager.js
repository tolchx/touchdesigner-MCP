 ...
 */
import { ensureKnowledgeLoaded, getOpsIndex, getPopsIndex, } from "./knowledgeCache.js";
 ...
 *                op.appendMenu(name, ...)
 */
export const PARAMETER_STYLE_MAP = {
    TOP: {
        style: "direct",
        description: "Standard parameter access via op.par.X assignment. Both .par.X = val and .par.X.val = val work.",
        setValueExample: "op.par.Noiseamplitude = 0.5",
        setExpressionExample: "op.par.Noiseamplitude.expr = 'absTime.frame / 100'",
    },
    CHOP: {
        style: "direct",
        description: "Standard parameter access. CHOPs frequently use channel expressions on sample/value parameters.",
        setValueExample: "op.par.Value0v = 0.8",
        setExpressionExample: "op.par.Value0v.expr = 'op(\"wave1\")[\"chan1\"]'",
    },
    SOP: {
        style: "direct",
        description: "Standard parameter access. Multi-parameter attributes use .par.Tuplet, .par.Tuplet2, etc.",
        setValueExample: "op.par.tx = 2.5",
        setExpressionExample: "op.par.tx.expr = 'me.time.frame * 0.1'",
    },
    DAT: {
        style: "direct",
        description: "Standard parameter access. Text content set via .text property, not .par.",
        setValueExample: "op.par.file = '/path/to/file.txt'",
        setExpressionExample: "op.text = op('table1').text",
    },
    POP: {
        style: "pop-custom",
        description: "POP operators manage custom parameters via append* methods. " +
    ...
    COMP: {
        style: "direct",
        description: "Standard parameter access. Components can define custom parameter pages via extension scripts.",
        setValueExample: "op.par.W = 1280",
        setExpressionExample: "op.par.W.expr = 'me.parent().par.W * 0.5'",
    },
    MAT: {
        style: "direct",
        description: "Standard parameter access. Material parameters include color pickers, textures, and uniforms.",
...
export const POP_PARAMETER_STYLE = {
     ...
     */
    signatures: {
        appendFloat: "appendFloat(name: str, label: str, order: int = 0, size: int = 1, " +
            "default: float = 0, min: float = -1, max: float = 1, " +
            "clampMin: bool = False, clampMax: bool = False) -> None",
        appendInt: "appendInt(name: str, label: str, order: int = 0, size: int = 1, " +
            "default: int = 0, min: int = -1, max: int = 1, " +
            "clampMin: bool = False, clampMax: bool = False) -> None",
        appendString: "appendString(name: str, label: str, order: int = 0, default: str = '') -> None",
        appendMenu: "appendMenu(name: str, label: str, order: int = 0, default: str = '', " +
    ...
    inputManagement: {
        ...
        description: "POP nodes chain sequentially: each POP outputs to the next POP's " +
...
export function ensureCatalogLoaded() {
    ...
    const opsIndex = getOpsIndex();
    const popsIndex = getPopsIndex();
    const entries = [];
 ...
 */
export function getCreationDefaults(opType) {
    ensureCatalogLoaded();
 ...
 */
export function isExperimental(opType) {
    ensureCatalogLoaded();
 ...
 */
export function getPaletteType(opType) {
    ensureCatalogLoaded();
    const entry = _catalogByType.get(opType.toLowerCase());
    if (entry)
        return entry.paletteType;
 ...
 */
export function getFamily(opType) {
    ensureCatalogLoaded();
    const entry = _catalogByType.get(opType.toLowerCase());
    if (entry)
        return entry.family;
 ...
 */
export function getParameterStyle(familyOrOpType) {
 ...
 */
export function listByFamily(family, includeExperimental = true) {
    ensureCatalogLoaded();
...
export function searchCatalog(query, options = {}) {
    ensureCatalogLoaded();
    const { family, includeExperimental = true, limit = 50, } = options;
    ...
    for (const entry of _catalogEntries) {
        if (family && entry.family !== family)
            continue;
        if (!includeExperimental && entry.isExperimental)
            continue;
        const haystack = [
            entry.opType,
            entry.label,
 ...
 */
export function getCatalogCountsByFamily() {
    ensureCatalogLoaded();
    const counts = {};
...
export function getAllFamilies() {
    return Object.keys(FAMILY_MAP);
}
/**
 * Convenience: get FamilyMeta for a family.
 */
export function getFamilyMeta(family) {
