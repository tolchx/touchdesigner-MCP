...
// mcp/src/popsBuild.ts
import fs from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
var __dirname = path.dirname(fileURLToPath(import.meta.url));
var DATA_ROOT = path.resolve(__dirname, "..", "data", "pops");
var OPERATORS_DIR = path.join(DATA_ROOT, "operators");
var INDEX_PATH = path.join(DATA_ROOT, "index.json");
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const isExperimental = typeof doc.experimental === "boolean" ? doc.experimental : file.includes("Experimental");
      operators.push({
