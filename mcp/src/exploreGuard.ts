...
export function normalizeScope(p?: string | null): string {
  const s = (p ?? "").trim();
  return s === "" ? "/" : s;
}
...
export function clampLimit(
  ...
  def = EXPLORE_DEFAULT_LIMIT,
): number {
  if (limit === undefined || limit === null || Number.isNaN(limit)) return def;
...
export class ExploreTimeoutError extends Error {
  readonly kind: TDErrorKind = "timeout";
  ...
  constructor(tool: string, scope: string, timeoutMs: number) {
    const hint =
      `'${tool}' no respondió en ${timeoutMs} ms. En redes grandes esto significa que TD ` +
...
export async function runBounded<T>(
  ...
  scope: string,
  fn: () => Promise<T>,
  timeoutMs: number = EXPLORE_DEFAULT_TIMEOUT_MS,
): Promise<T> {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(
          () => reject(new ExploreTimeoutError(tool, scope, timeoutMs)),
          timeoutMs,
        );
      }),
    ]);
...
export const scopeSchema = z
  ...
  .optional()
  .describe(
    "Scope to explore. Defaults to '/'. Prefer a narrow path: sweeping a large " +
