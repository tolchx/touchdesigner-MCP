...
interface RawPopEntry {
  type: string;
  name: string;
  family: string;
  created_live: boolean;
  inputs: number;
  outputs: number;
  param_count: number;
  live_params: string[];
...
let cachedKnowledge: Map<string, PopOperatorInfo> | null = null;
 ...
 */
export function loadPopKnowledge(): Map<string, PopOperatorInfo> {
  if (cachedKnowledge) return cachedKnowledge;
  const map = new Map<string, PopOperatorInfo>();
  const path = resolveKnowledgePath();
  if (!path) {
    console.warn(
      "[popKnowledge] pop_operators.json not found — parameter validation against the knowledge base is disabled",
    );
    return map;
  }
  try {
    const raw = JSON.parse(fs.readFileSync(path, "utf-8"));
    const entries: RawPopEntry[] = Array.isArray(raw?.operators)
      ? raw.operators
      : [];
    for (const e of entries) {
      if (!e?.type || !Array.isArray(e.live_params)) continue;
      const info: PopOperatorInfo = {
        type: e.type,
    ...
    console.warn(
      "[popKnowledge] Failed to load pop_operators.json:",
...
export function getPopInfo(opType: string): PopOperatorInfo | undefined {
  if (!opType) return undefined;
  return loadPopKnowledge().get(opType.toLowerCase());
}

/** All known POP type names (canonical, e.g. "circlePOP"). */
export function listPopTypes(): string[] {
  const types = new Set<string>();
  for (const info of loadPopKnowledge().values()) types.add(info.type);
  return [...types].sort();
}
 ...
 */
export function suggestParameterNames(
  knownNames: string[],
  badName: string,
  max: number = 5,
): string[] {
  ...
  const scored = knownNames.map((name) => {
    ...
    if (lower.startsWith(needle) || lower.includes(needle)) score -= 2;
    return { name, score };
  });
 ...
 */
export function formatUnknownParameterError(
  opType: string,
  ...
  return (
    `Unknown parameter(s) for ${opType}: ${parts.join(", ")}. ` +
