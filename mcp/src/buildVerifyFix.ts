 ...
 */
async function healthcheckPath(
  client: TDClient,
  path: string,
  recurse: boolean = false,
): Promise<{ ok: boolean; issueCount: number; operators: any[] }> {
  try {
 ...
 */
async function attemptRewire(
  client: TDClient,
  sourcePath: string,
  targetPath: string,
  inputIndex: number,
): Promise<RewireAttempt> {
  const strategies = [
 ...
 */
export async function buildVerifyFix(
  ...
  const issues = health.operators
    .filter((o: any) => o.hasIssues)
    .map((o: any) => `${o.path}: ${o.errors || o.warnings || "unknown"}`);
 ...
 */
export async function verifyAndFixConnections(
  client: TDClient,
  connections: Array<{
    sourcePath: string;
    targetPath: string;
...
): Promise<{ succeeded: number; failed: RewireAttempt[]; fixed: RewireAttempt[] }> {
  const failed: RewireAttempt[] = [];
  const fixed: RewireAttempt[] = [];
  let succeeded = 0;
  ...
  for (const conn of connections) {
    ...
    if (verified) {
      ...
      // Attempt rewire
      const rewire = await attemptRewire(
        client,
        conn.sourcePath,
        conn.targetPath,
        conn.inputIndex,
 ...
 */
export async function postGraphValidation(
  client: TDClient,
  path: string,
): Promise<VerifyResult> {
