...
export function getInputCount(opType: string, opData?: any): number {
  ...
  for (const [pattern, count] of Object.entries(MULTI_INPUT)) {
    if (key.includes(pattern.toLowerCase())) return Math.abs(count);
  }

  // Check operator doc inputs
  if (opData?.inputs && Array.isArray(opData.inputs)) {
    const docInputs = opData.inputs.filter(
      (i: any) => i.description && i.description !== "-"
    );
    if (docInputs.length > 0) return docInputs.length;
  }
...
export async function main() {
  ...
  if (fs.existsSync(opsIndexPath)) {
    ...
    for (const op of operators) {
      ...
      if (!opType) {
        ...
        opType = (op.pageSlug || "").replace(/_/g, "").replace(/^(.)/, (c: string) => c.toLowerCase());
  ...
  if (fs.existsSync(popsIndexPath)) {
    ...
    for (const op of operators) {
      ...
      if (!opType) {
        opType = (op.pageSlug || "").replace(/_/g, "").replace(/^(.)/, (c: string) => c.toLowerCase());
        if (!/POP$/.test(opType)) opType = opType + "POP";
      }
