 ...
 */
export const MULTI_INPUT_PATTERNS: Record<string, { min: number; max: number }> = {
  // TOPs
 ...
 * and pattern tables.
 */
export function inferOpTopology(
  opType: string,
  opData: any,
): OpTopology {
    ...
    if (popInfo && !popInfo.recommendedForNetworks) {
      switch (popInfo.validationCategory) {
        case "error_con_input":
          warnings.push(
            "NOT recommended for generated networks: reported TD errors() when cooked even WITH a correct source (live matrix evidence) — prefer a validated alternative",
          );
          break;
        case "sin_geometria_con_input":
          warnings.push(
            "NOT recommended for generated networks: cooks clean but produces 0 points with a POP source (cross-family input or output node)",
          );
          break;
        case "no_creable":
          warnings.push(
            "NOT recommended: cannot be created via create() in this TD build",
          );
          break;
        default:
 ...
 */
export function buildTopologyCatalog(): Map<string, OpTopology> {
