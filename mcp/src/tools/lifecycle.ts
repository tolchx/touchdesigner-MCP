...
export function registerLifecycleTools(server: McpServer, client: TDClient) {
  ...
  server.registerTool(
    "td_memory_save",
    {
      title: "Save Memory",
      description:
        "Save a persistent memory entry with key, content, and optional tags. Memories survive across sessions.",
      inputSchema: {
  ...
  server.registerTool(
    "td_memory_recall",
    {
      title: "Recall Memory",
      description:
        "Search saved memories by query text. Returns scored results from persistent memory store.",
      inputSchema: {
