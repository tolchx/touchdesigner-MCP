...
export function registerTdnTools(server: McpServer, client: TDClient) {
  // ---------------------------------------------------------------------------
  // td_tdn_export
  // ---------------------------------------------------------------------------
  server.registerTool(
    "td_tdn_export",
    {
      ...
      description:
        "Export a TouchDesigner operator/network to a .tdn (TouchDesigner Network) JSON file. " +
  ...
  server.registerTool(
    "td_tdn_import",
    {
      ...
      description:
        "Import a .tdn (TouchDesigner Network) JSON file into TouchDesigner. " +
  ...
  server.registerTool(
    "td_tdn_diff",
    {
      ...
      description:
        "Compare a live TouchDesigner network against a saved .tdn file. " +
  ...
  server.registerTool(
    "td_tdn_git_setup",
    {
      ...
      description:
        "Auto-configure git for TouchDesigner Network (.tdn) version control. " +
      ...
      inputSchema: {
        repo_path: z
          ...
          .optional()
          .describe(
            "Path to the git repository root. Defaults to the MCP server's working directory.",
          ),
        textconv_path: z
          .string()
          .optional()
          .describe(
            "Path to textconv.py relative to repo root. Defaults to 'mcp/src/tdn/textconv.py'.",
          ),
        add_to_gitignore: z
          .boolean()
          .optional()
          .default(false)
          .describe(
            "Also add *.tdn to .gitignore (default: false — .tdn files should be tracked).",
          ),
        global: z
          .boolean()
          .optional()
          .default(false)
          .describe(
            "Configure git globally (--global flag) instead of per-repo.",
          ),
      },
    },
      ...
      try {
        ...
        if (isGitRepo) {
          ...
          if (existingContent.includes(tdnLine)) {
            ...
            steps.push(
              `✅ Wrote .gitattributes with '${tdnLine}'`,
            );
          }
          ...
          try {
            ...
            try {
              current = execSync(
                `git config${globalFlag} diff.tdn.textconv`,
            ...
            if (current === textconvValue) {
              ...
              execSync(
                `git config${globalFlag} diff.tdn.textconv "${textconvValue}"`,
                { cwd: repoRoot, stdio: "pipe", timeout: 5000 },
              );
