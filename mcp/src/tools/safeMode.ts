...
export function registerSafeModeTools(server: McpServer, client: TDClient) {
  // ---------------------------------------------------------------------------
  ...
  server.registerTool(
    "td_safe_plan",
    {
      ...
      description:
        "Safe mode workflow: diagnose the current TD state before making changes. " +
      ...
      inputSchema: {
        path: z
          ...
          .optional()
          .describe(
            "Specific path to diagnose. If omitted, uses *here (current network) automatically."
          ),
        include_children: z
          .boolean()
          .optional()
    ...
    async ({ path: opPath, include_children }) => {
      try {
        ...
        try {
          ...
          ctx = (spatial as any)?.context ?? null;
 ...
 */
function generateSafePlan(result: any): any {
  const plan: any = {
    status: "ready",
    recommendations: [],
    warnings: [],
  ...
  if (!paths.target || paths.target === "/") {
    plan.warnings.push(
      "No specific target path resolved. Consider navigating to a specific network first."
    );
  }
  ...
  if (issues?.hasIssues) {
    ...
    plan.recommendations.push(
      `Found ${total} issue(s) in the network. Review before making changes.`
    );

    if (counts.expression_error) {
      plan.recommendations.push(
        `${counts.expression_error} expression error(s) detected — run td_validate with auto_fix=true to fix automatically.`
      );
    }
    if (counts.glsl_error) {
      plan.recommendations.push(
        `${counts.glsl_error} GLSL shader error(s) — check shader source code.`
      );
    }
    if (counts.cook_loop) {
      plan.warnings.push(
        `${counts.cook_loop} cook loop(s) detected — these may be pre-existing TD infrastructure issues.`
      );
    }
    if (counts.missing_file) {
      plan.recommendations.push(
        `${counts.missing_file} missing file(s) — verify file paths before proceeding.`
      );
    }
  } else {
    plan.status = "healthy";
    plan.recommendations.push(
      "Network is healthy. Safe to proceed with modifications."
    );
  }
  ...
  if (paths.selected?.length > 0) {
    plan.recommendations.push(
      `${paths.selected.length} operator(s) selected — modifications will target: ${paths.selected.join(", ")}`
    );
  }

  if (paths.focusedOperator) {
    plan.recommendations.push(
      `Current operator: ${paths.focusedOperator.path} (${paths.focusedOperator.opType})`
    );
  }
