...
export const GLSL_TOP_RECIPES: GlslTopRecipe[] = [
  {
    id: "top-circle-sdf",
    ...
    liveCheck:
      "numpyArray(): centro blanco (R>0.9), esquinas negras (R<0.1), " +
  ...
  },
  {
    ...
    resolution: { w: 256, h: 256 },
    liveCheck:
      "numpyArray(): distribución de grises (min<0.2, max>0.8, no plano), " +
  ...
  },
  {
    ...
    resolution: { w: 256, h: 256 },
    liveCheck:
      "numpyArray(): nube fractal suave (std entre 0.05 y 0.3, sin bandas duras); " +
  ...
  },
  {
    ...
    resolution: { w: 256, h: 256 },
    liveCheck:
      "numpyArray(): pico blanco en el centro de cada celda — contar cruces de " +
  ...
  },
  {
    ...
    resolution: { w: 256, h: 256 },
    liveCheck:
      "numpyArray(): perfil radial oscilante — muestrear a lo largo del radio " +
  ...
  },
  {
    ...
    resolution: { w: 256, h: 256 },
    liveCheck:
      "WIRING ONLY en suite (Regla TOP 10: el buffer no avanza con cook scripteado): " +
  ...
  },
  {
    ...
    resolution: { w: 256, h: 256 },
    liveCheck:
      "WIRING ONLY (T10): compila y cablea glsl→feedback→glsl sin errores; evolución " +
 ...
 * Python 3.9-safe.
 */
export function buildGlslTopRecipeCode(recipe: GlslTopRecipe, parentPath: string, prefix: string): string {
  const shaderLiteral = JSON.stringify(recipe.glsl);
  const safePrefix = sanitizeNodeName(prefix);
  const uniLines = (nodeVar: string, indent: string) =>
    recipe.uniforms
      .map(
        (u) =>
          `${indent}_set_par(${nodeVar}, ${JSON.stringify(u.namePar)}, ${JSON.stringify(u.uniformName)})\n` +
...
export function validateAllRecipes(): Array<{ id: string; errors: string[] }> {
...
export function sanitizeNodeName(id: string): string {
