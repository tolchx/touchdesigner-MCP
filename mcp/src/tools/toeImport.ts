...
export function parseNFile(content: string, name: string): ToeNode | null {
 ...
 */
export function parseParmFile(content: string): Record<string, string> {
...
export function dumpTypeToPyClass(rawType: string): string | null {
  const m = FAMILY_RE.exec(rawType.trim());
  if (!m) return null;
  const family = m[1];
  if (family === "PANEL") return null; // panel comps are not created this way
  const base = m[2];
  return `${base}${family}`;
}
 ...
 */
export function parseToeDirScope(scopeDir: string): ToeDump {
  const dump: ToeDump = { scope: path.basename(scopeDir), nodes: [], wires: [] };
  const files = fs.readdirSync(scopeDir);

  for (const f of files) {
    if (!f.endsWith(".n")) continue;
    const name = f.slice(0, -2);
    const node = parseNFile(
      fs.readFileSync(path.join(scopeDir, f), "utf8"),
      name,
    );
    if (!node) continue;
...
export function buildImportCode(dump: ToeDump, containerName: string, parentPath: string): string {
...
export interface NodeCheck {
  node: string;
  checks: { present: boolean; type: boolean; position: boolean; inputs: boolean; clean: boolean };
  geometry: { numPoints: number | null; numPrims: number | null };
  errors: string;
}

/** Compare TD's per-node verification payload against the dump. Pure function. */
export function verifyAgainstDump(
  dump: ToeDump,
  verification: Record<string, any>,
): { checks: NodeCheck[]; allOk: boolean; geometrySignature: Record<string, { numPoints: number | null; numPrims: number | null }> } {
  ...
  for (const n of dump.nodes) {
    ...
    const gotInputs: string[] = Array.isArray(v.inputs) ? v.inputs : [];
    checks.push({
      node: n.name,
      checks: {
        present: true,
        ...
        inputs:
          expectedInputs.length === gotInputs.length &&
  ...
  const allOk = checks.every(
    (c) => c.checks.present && c.checks.inputs && c.checks.clean,
  );
  const geometrySignature: Record<string, { numPoints: number | null; numPrims: number | null }> = {};
  for (const c of checks) geometrySignature[c.node] = c.geometry;
  return { checks, allOk, geometrySignature };
}

// ─── MCP tool registration ──────────────────────────────────────────────────

export function registerToeImportTools(server: McpServer, client: TDClient) {
  server.registerTool(
    "td_import_toe_dir",
    {
      title: "Import Toe_Expand .toe.dir network",
      description:
        "Rebuild a TouchDesigner network from an expanded Toe_Expand .toe.dir " +
