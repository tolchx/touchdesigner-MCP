...
function parseBridgeError(e: unknown): { status?: number; body?: any; message: string } {
...
export function registerBridgeHistoryTools(server: McpServer, client: TDClient) {
  // ---------------------------------------------------------------------------
  // td_undo
  // ---------------------------------------------------------------------------
  server.registerTool(
    "td_undo",
    {
      ...
      description:
        "Revert the most recent bridge write request (parameters/set, create, delete, connect, disconnect) via POST /undo. Each undo reverts ONE complete request. /exec code is NOT recorded. A new write clears the redo branch. History is capped at 50 entries, FIFO. Returns success:false with a hint when there is nothing to undo.",
      inputSchema: {},
    },
    async () => {
      try {
  ...
  server.registerTool(
    "td_redo",
    {
      ...
      description:
        "Re-apply the most recently undone bridge write via POST /redo. Only available immediately after td_undo (any new write discards the redo branch). Returns success:false with a hint when there is nothing to redo.",
      inputSchema: {},
    },
    async () => {
      try {
  ...
  server.registerTool(
    "td_history",
    {
      ...
      description:
        "List the bridge's reversible write operations via GET /history: one-line description per entry plus kind (parameters|ops|wiring). Shape: {maxDepth, canUndo, canRedo, undo[], redo[]}. Use td_undo to revert the newest entry.",
      inputSchema: {},
    },
    async () => {
      try {
