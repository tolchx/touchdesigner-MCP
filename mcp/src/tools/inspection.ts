...
export function registerInspectionTools(server: McpServer, client: TDClient) {
  ...
  server.registerTool(
    "td_get_errors",
    {
      title: "Get Errors",
      description:
        "Get errors and warnings from a TouchDesigner operator or entire network. Reads errors WITHOUT cooking (non-mutating); pass forceCook=true to materialize fresh errors (mutates the network by cooking).",
      inputSchema: {
        ...
        forceCook: z
          ...
          .default(false)
          .describe(
            "Opt-in: force-cook each operator to materialize fresh errors (MUTATES the network; default false = read-only)"
          ),
      },
    },
  ...
  server.registerTool(
    "td_healthcheck",
    {
      ...
      description:
        "Validate a TouchDesigner operator/network, reporting errors, warnings and per-operator issues. Non-mutating by default (reads errors without cooking); pass forceCook=true to opt into force-cooking (mutates the network).",
      inputSchema: {
  ...
  server.registerTool(
    "td_pop_inspect",
    {
      ...
      description:
        "Read particle data from a POP operator: point/prim/vert counts, attribute list " +
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        const result = await runBounded("td_spatial_context", "-", () =>
  ...
  server.registerTool(
    ...
    async ({ path: opPath }) => {
      try {
        const scope = normalizeScope(opPath);
        const result = await runBounded("td_explore_project", scope, () =>
          client.exploreProject(scope),
        );
        return ok(result);
      } catch (e: any) {
        return err(e);
...
try:
    ...
    print(json.dumps({'success': False, 'error': str(e)}))`;
        ...
        const result = await runBounded("td_compare_networks", `${safeA} vs ${safeB}`, () =>
          client.execute(code, "/"),
        );
        if (!result.success) {
