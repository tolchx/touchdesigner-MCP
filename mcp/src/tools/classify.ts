...
export function classifyIssue(issue: {
  ...
  for (const [cat, patterns] of CATEGORY_PATTERNS) {
    ...
    if (category !== "needs_manual") break;
  }
 ...
 */
export function classifyAllIssues(
  operators: Array<{ path: string; errors?: string; warnings?: string }>
): ClassificationResult {
  const classified = operators.map(classifyIssue);

  const counts: Record<string, number> = {};
  const summary: Record<string, ClassifiedIssue[]> = {};
 ...
 */
export function buildClassifyPythonCode(healthJson: string): string {
