 ...
 */
export function registerGlslTopTools(server: McpServer, client: TDClient) {
  server.registerTool(
    "td_glsl_top_analyze",
    {
      title: "Analyze GLSL TOP Shader",
      description:
        "Statically analyze a GLSL TOP (pixel) shader against the verified rules " +
      ...
      inputSchema: {
        ...
        creation_code: z
          ...
          .optional()
          .describe(
            "Optional Python creation code (from buildGlslTopRecipeCode or hand-written) — enables the T4 uniform0name check"
          ),
      },
    },
  ...
  server.registerTool(
    "td_glsl_top_recipe",
    {
      ...
      description:
        "Create one of the verified GLSL TOP visual recipes (Book of Shaders " +
      ...
      inputSchema: {
        recipe_id: z
          .string()
          .describe(
            "One of: " + GLSL_TOP_RECIPES.map((r) => r.id).join(", ")
          ),
        parent_path: z
          .string()
          .default("/project1")
    ...
    async ({ recipe_id, parent_path, name }) => {
      try {
        ...
        if (!recipe) {
          return err(
            "Unknown recipe_id '" + recipe_id + "'. Available: " +
              GLSL_TOP_RECIPES.map((r) => r.id).join(", ")
          );
        }
        ...
        if (problems) {
          return err(
            "Recipe failed its own safety net (bug — please report): " +
              problems.join("; ")
          );
        }
        ...
        if (!result.success) {
          return err(
            "Execution failed: " + (result.error?.message ?? result.error)
          );
        }
        return ok({
          message:
            "Recipe '" + recipe_id + "' created as /" +
