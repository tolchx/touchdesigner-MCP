...
export function registerRunnerTool(server: McpServer, client: TDClient) {
  // -----------------------------------------------------------------------
  ...
  server.registerTool(
    "td_run_test",
    {
      ...
      description:
        "Execute a legacy TouchDesigner MCP test script from the mcp/tests/ directory. " +
      ...
      inputSchema: {
        name: z
          .string()
          .describe(
            "Test file name (without .mjs extension), e.g. 'legacy_test_30systems'"
          ),
        args: z
          .string()
          .optional()
