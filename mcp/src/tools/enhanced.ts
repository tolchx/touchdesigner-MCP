...
export function registerEnhancedTools(server: McpServer, client: TDClient) {
  ...
  server.registerTool(
    "td_resolve_operator",
    {
      ...
      description:
        "Convert natural language descriptions to exact TouchDesigner operator types. " +
  ...
  server.registerTool(
    "td_get_template",
    {
      ...
      description:
        "Get a pre-built network template with exact port-level wiring, parameters, and Python builder code. " +
  ...
  server.registerTool(
    "td_get_recipe",
    {
      ...
      description:
        "Get a production-ready builder recipe with paste-ready Python code, gotchas, and wiring. " +
  ...
  server.registerTool(
    "td_search_knowledge",
    {
      ...
      description:
        "Full-text search across all TouchDesigner operator documentation (FTS5 + BM25 ranking). " +
  ...
  server.registerTool(
    "td_catalog",
    {
      ...
      description:
        "Browse the operator catalog: list by family, search by name, get creation defaults. " +
    ...
    async ({ action, family, query, opType }) => {
      try {
        ...
        if (action === "get" && opType) {
          ...
          if (!entry) return err(new Error(`Operator "${opType}" not found in catalog.`));
  ...
  server.registerTool(
    "td_get_family_hints",
    {
      ...
      description:
        "Get family inference hints: which TD operator family (TOP/CHOP/SOP/POP/etc.) matches your keywords. " +
  ...
  server.registerTool(
    "td_patch_plan",
    {
      ...
      description:
        "Plan a complex network patch with pre-turn context injection. " +
    ...
    async ({ prompt, target_path, dry_run }) => {
      try {
        const result = await runPatchWorkflow(
          client,
          prompt,
          target_path || "/",
          { dryRun: dry_run ?? true },
  ...
  server.registerTool(
    "td_patch_preview",
    {
      title: "Preview Patch",
      description:
        "Preview exactly what nodes will be created and how they'll be connected. " +
  ...
  server.registerTool(
    "td_patch_apply",
    {
      ...
      description:
        "Apply a planned patch with automatic undo-block wrapping and rollback on failure. " +
  ...
  server.registerTool(
    "td_patch_variations",
    {
      ...
      description:
        "Generate alternative approaches for a patch (Minimal, Alternative operators, Parallel chains). " +
  ...
  server.registerTool(
    "td_complexity_check",
    {
      ...
      description:
        "Analyze a prompt for complexity: returns tier (basic/standard/pro), " +
