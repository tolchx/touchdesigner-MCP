...
interface SyntacticCheckResult {
  ok: boolean;
  checks: {
    pythonSyntax: { passed: boolean; errors: string[]; warnings: string[] };
    pathSafety: { passed: boolean; errors: string[]; warnings: string[] };
    dependencyCheck: { passed: boolean; errors: string[]; warnings: string[] };
    jsonIntegrity: { passed: boolean; errors: string[]; warnings: string[] };
  };
  summary: string;
  suggestions: string[];
}
...
function validatePythonSyntax(code: string): { passed: boolean; errors: string[]; warnings: string[] } {
  ...
  if (bareMatches && bareMatches.length > 0) {
    ...
    warnings.push(
      `Bare math functions detected: ${unique.join(", ")} — these need 'math.' prefix in TD (e.g. math.sin). Auto-fix available via td_validate.`,
    );
  }
  ...
  try {
    ...
    if (version) {
      ...
      if (match) {
        ...
        if (year < 2025) {
          warnings.push(
            `TouchDesigner version ${version} may be below recommended 2025+ — some POP features may not be available`,
          );
        }
      }
    }
...
function validateJsonIntegrity(
  ...
  for (let i = 0; i < jsonStrings.length; i++) {
    ...
    if (raw.includes("```json") || raw.includes("```")) {
      errors.push(
        `JSON string ${i + 1} contains markdown code fences — LLM output should be raw JSON`,
      );
    }
...
function validateCrossFamilyConnections(
  ...
  for (const [src, tgt] of crossFamilyPairs) {
    ...
    warnings.push(
      `Cross-family connection detected: ${src} → ${tgt}. ` +
  ...
  while ((popMatch = popCreatePattern.exec(code)) !== null) {
    ...
    if (popType === "particlePOP" && !code.includes("particlesupdatepop")) {
      warnings.push(
        "particlePOP created but particlesupdatepop parameter not set — " +
    ...
    if (popType === "noisePOP" && !code.includes("inputConnectors[0].connect")) {
      warnings.push(
        "noisePOP may need an input connection — it cannot run standalone.",
      );
    }
  }
...
export function registerSyntacticCheckTools(server: McpServer, client: TDClient) {
  // ---------------------------------------------------------------------------
  // td_syntactic_check
  // ---------------------------------------------------------------------------
  server.registerTool(
    "td_syntactic_check",
    {
      ...
      description:
        "Validate Python code, TD paths, JSON, and environment dependencies BEFORE sending to TD. " +
    ...
    async ({ code, paths, json_strings, check_dependencies }) => {
      try {
        const result: SyntacticCheckResult = {
          ok: true,
          checks: {
        ...
        if (code) {
          ...
          if (!result.checks.pythonSyntax.passed) result.ok = false;
