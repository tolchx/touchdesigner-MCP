...
class LlmHttpError extends Error {
    ...
    body;
    constructor(options) {
        super(`${options.provider} HTTP ${options.status} ${options.statusText}: ${options.body}`);
...
async function fetchWithRetry(url, init, options) {
    for (let attempt = 0; attempt <= options.maxRetries; attempt++) {
        ...
        if (response.ok)
            return response;
        const shouldRetry = options.retryOnStatuses.includes(response.status) &&
...
export class OllamaFallbackLlmClient {
    ...
    async generateText(input) {
        ...
        for (const model of this.models) {
            try {
                const client = new OllamaLlmClient({ model, baseUrl: this.baseUrl });
                return await client.generateText(input);
            }
            catch (error) {
                lastError = error;
...
export function createLlmClientFromEnv() {
    ...
    if (provider === "gemini") {
        ...
        if (modelsFromList.length > 0) {
            return new GeminiFallbackLlmClient({ apiKey, models: modelsFromList, baseUrl });
        }
        if (model.includes(",")) {
            const models = model
                .split(",")
    ...
    const responseText = process.env.MOCK_LLM_RESPONSE ??
