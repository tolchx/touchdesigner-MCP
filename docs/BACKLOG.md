...
- [x] 47. **`td_measure` — muestrear magnitudes reales antes de fijar escalares. CERRADO 24/09/26 con aceptación EN VIVO.** `mcp/src/tools/measure.ts` + registro en `server.ts`, 20 tests offline (`mcp/test/measure.test.js`, handler real sobre un doble de McpServer, no un espejo) y 9 chequeos en vivo (`scripts/live/twozero_measure_live.mjs`, `ALL_OK=true`). Medido en el proyecto abierto: CHOP `u,v` bipolar; **buffer de 15.000 muestras** (`/project1/LaGSplat/splats`, `P_1` de -0.21 a 9.43 → la tool sugiere `scale=0.1036`, `offset=0.0221`) con cross-check contra una cuenta hecha por separado DENTRO de TD (diferencia 3.8e-5, tolerancia 1% porque el scriptCHOP recocina entre las dos lecturas); TOP 40x40 aspect 1; POP 63 puntos/3 prims/66 verts; DAT 1 fila; SOP → error accionable. **Tres bugs reales que solo aparecieron con TD arriba:** (a) `api/src/index.ts readChop` llamaba `t.channel()` — que NO EXISTE en TD — y devolvía `channels: {}` SIEMPRE y en silencio: ahora usa `t.chan()`/`t[nombre][i]` y **falla explícito** si el CHOP declara canales y no se leyó ninguno (regresión cubierta por `mcp/test/readPathCodegen.test.js`); (b) el mismo archivo asignaba `t.numPoints` sin llamarlo, y en este build son MÉTODOS: `json.dumps` moría con "not JSON serializable" (arreglado en el cliente TS y en `toe/src/TouchDesignerAPI.py`, que tenía la copia); (c) `execute()` devuelve el JSON dentro de `stdout`, no en `.data`, así que el TOP reportaba `resolution: null` (ahora `parseExecJson`). README actualizado (107 tools, conteos 1347/305).
...
- [x] 50. **ACEPTADO EN VIVO 24/09/26** — cadena, portabilidad y guardrails contra TD 2025.32460 real (proyecto abierto de 26.340 operadores). Scripts: `scripts/live/twozero_chain_live.mjs` (`ALL_OK=true`) y `scripts/live/twozero_http_live.py` (`ALL_OK=true`); evidencia versionada en `docs/discord-twozero/evidence/`. Hallazgos que CAMBIARON código: (a) el toggle global de cooking NO es legible en este build (`app.cooking`, `app.cook`, `ui.cook`, `project.cook` → AttributeError), así que `runtime.cooking` queda `null` honesto con `cooking_source: null`, y se suman `cooking_allowed` (`op('/').allowCooking`) y `target_fps` (`me.time.rate` — rate objetivo, NO el fps medido, que sale del chequeo de performance; el campo se llamaba `fps` y era un nombre mentiroso); (b) los 31 issues del proyecto convertían en DEGRADED una cadena 4/4 OK → los errores del proyecto pasan a `warnings` y el veredicto mide operabilidad; (c) la resolución portable no contemplaba que el `.toe` vive en `toe/` (los assets están un nivel arriba): `/dashboard` y `/web2touch` servían el cartel de "Not found" con HTTP 200 → corregido probando también los padres, verificado con 222 KB de HTML real y el asset JS con su MIME; (d) `td_find` sin acotar devuelve 32.118 coincidencias en 302 ms con paginación real (100 resultados + `truncated: true`, 12 KB de payload, sin bomba de contexto): el cuelgue del competidor NO se reproduce acá, el presupuesto de 20 s queda como red de seguridad. Nota de diseño: apagar el cooking global para "probar" DEGRADED es autodestructivo (si TD deja de cocinar, el WebServer DAT deja de atender y no hay con qué volver a prenderlo) — el estado de cocción se LEE, nunca se provoca.

- [ ] 51. `mcp/setup/toe_extension.py` (65.970 chars, 1.645 líneas) NO es un espejo de `toe/src/TouchDesignerAPI.py` (5.353 líneas): es otra implementación, v4.0.0, class-based (`def _handle_info(self)` devolviendo `{status, body, headers}`), sin rutas estáticas, sin `probe_runtime`, sin `runtime`/`bridge` en `/info`. Verificado el 24/09/26: no comparte esas piezas con el archivo que corre en vivo (el `/info` del bridge real sí expone `runtime` y `bridge`). Riesgo: el `.tox` de `mcp/setup/` que se entregue a terceros lleva la conducta vieja (sin estado de cocción, sin veredicto de cadena, sin guardrails). Fix: decidir si el setup se RELICUA desde `toe/src` o si se retira; mientras tanto no asumir paridad entre ambos.

- [x] 52. **CERRADO 24/09/26.** Suite `tests/test_comprehension_td.py` (13 tests) + fake fiel `tests/fake_td/fake_td.py` (ejecuta el código REAL del bridge contra un modelo de TD construido con lo MEDIDO). Prueba de valor: reintroducir el bug de `numPoints` en el bridge hace caer 2 tests; sin el bug, 13/13. Lo que NO cubre offline (y queda EN VIVO): render no degenerado, escala/presupuesto, y los bloques declarados en NO_MATERIALIZABLES. Pendiente original: **Suite de COMPRENSIÓN de TD — que el MCP no solo "no explote", sino que entienda lo que hace adentro.** Pedido por Tolch el 24/09/26 a partir de lo que pasó con el ítem 47: los dos bugs más caros (`t.channel()` inexistente devolviendo `channels:{}` en silencio, `numPoints` como método rompiendo `json.dumps`) NO los detectaba ninguna suite: había 1324 tests verdes y las dos rutas rotas desde siempre. El patrón que falta: **verificar contra la realidad de TD, no contra el eco de la propia tool**. Brief `52_comprension_td.txt` con las 12 familias de prueba y el arnés (fake del API de TD en Python para EJECUTAR el código generado + cross-check en vivo cuando TD está arriba). Criterio de aceptación: cada familia con al menos un test que HABRÍA FALLADO con los bugs conocidos (regresión real, no cobertura decorativa).
...
- [x] 53. **Loop de auto-mejora del MCP (loop engineering) — IMPLEMENTADO 24/09/26.** Método de [cobusgreyling/loop-engineering](https://github.com/cobusgreyling/loop-engineering) adaptado a Hermes; NO se instaló su CLI (sus `loop-init/audit/cost/gate` son genéricos de repos npm). Auditoría honesta contra su rubric de 10 secciones: el repo YA tenía 6 de los 7 bloques (scheduling con los crons del ciclo diario/canario/soak, skills, conectores MCP, maker≠checker con Freebuff + juez externo, y estado en `STATE.md`/BACKLOG/Mnemosyne); faltaba lo que hace que un loop se **auto-mejore** en vez de solo ejecutar una cola escrita a mano. Agregado: `LOOP.md` (mapa de loops, niveles, prioridad, handoff), `loop-constraints.md` (reglas vinculantes: alcance, prohibido simular verificación en vivo, no deshabilitar tests, un fix por corrida, máx 3 intentos, nada de `raise SystemExit` dentro de TD), `loop-budget.md` (cupos + kill switch), `gate.yaml` (denylist/maxFiles/maxLines/autoMergeAllowlist/requireGreenTests), `loop-run-log.md` (append-only), `loop-ledger.json` (intentos por item), y los dos scripts deterministas y **sin LLM**: `scripts/loop_triage.py` (descubrimiento: convierte señales reales — árbol sucio, drift de cola, suites en rojo, evidencia en vivo en rojo, tipos de error del log del cliente, items cerrados sin verificación, cola larga — en candidatos con evidencia, deduplicados por huella; `--audit` da puntaje 0-100) y `scripts/loop_gate.py` (freno mecánico; exit 0/2/4/5). Cron nuevo `TD-MCP · loop triage` (`3e44912ae9f1`, cada 4 h, script-only, silencioso: solo habla cuando hay candidatos nuevos). Probado en vivo: gate ALLOW con suites verdes y BLOCK sobre `toe/*.toe` y sobre auto-merge fuera del allowlist; cap de intentos cortando en el 4º; triage encontrando y luego deduplicando. **Loop Readiness medido: 89/100, nivel L3**, con UN gap declarado: la verificación de comportamiento ejecutable (item 52) — sin ella el verifier no distingue "la tool anda" de "la tool devolvió algo", así que el loop es L3 en maquinaria y L2 en verificación. NO se adoptaron worktrees (Freebuff trabaja en el árbol principal; mitigado con gate + juez). Kill switch: `loop-pause-all` o `loop: paused` en `STATE.md`.
...
- [x] 56. Cerrada 24/09/26 — las 5 tools de exploración restantes envueltas con normalizeScope/clampLimit/runBounded (`td_search` — la que colgaba TD según el Discord—, `td_explore_project`, `td_snapshot_scene`, `td_compare_networks`, `td_spatial_context`) más descripciones que le piden al modelo acotar el scope. test-GUARD en `mcp/test/exploreGuard.test.js`: escanea el FUENTE de `mcp/src/tools/*.ts`, empareja cada `runBounded("td_x")` con el `registerTool` que lo contiene y falla si una tool de la lista canónica (10 tools) queda sin envolver, si hay un runBounded huérfano o si el nombre no coincide con la tool. FAIL forzado verificado: quitando el envoltorio de td_search el test cae nombrándola; revertido. Node 1351/0 (4 tests nuevos), discover 532 OK 15.4s, gate ALLOW.
