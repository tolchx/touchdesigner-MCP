...
| 13 | GET | `/healthcheck?path=/…&recurse=0/1[&force_cook=0/1]` | `pars.path`, `pars.recurse`, `pars.force_cook` | `{path, recurse, forceCook, ok, issueCount, issues:[…], operators:[…]}`; NO-mutante por default (A3 fix 2026-09-22: sin cook; ítems llevan `cooked`/`pre_existing_errors`); `force_cook=1` opt-in cocina y lo declara | `_handle_healthcheck` |
| 14 | GET | `/get_errors?path=/…&recurse=0/1[&force_cook=0/1]` | `pars.path`, `pars.recurse`, `pars.force_cook` | idéntico a `/healthcheck` (alias, siempre recursivo por defecto, mismo `force_cook` opt-in) | `_handle_get_errors` → `_handle_healthcheck` |
...
| 35 | GET | `/verify?path=/…` | `pars.path` (query, no body) | `{path, operators_scanned, total_in_tree, errors:[… ≤200], error_count (FULL), warnings:[… ≤200], warning_count (FULL), total_connections, healthy, pop_stats, errors_truncated, warnings_truncated}` (A1 fix 2026-09-22: scan completo, cap solo de serialización declarado) | `_handle_verify` |
...
|---|----------|-------------------------------|-----------|-----------|
| A1 | `GET /verify` | **FIXED 2026-09-22**: el scan cubre TODOS los nodos (el cap 200 es solo de serialización de las listas `errors`/`warnings`); `error_count`/`warning_count` son el conteo FULL y las claves aditivas `errors_truncated`/`warnings_truncated` declaran el recorte. Verificado en vivo: 211 ops con error en #205 → `error_count=1, healthy=false, errors_truncated=false`. (Antes: cap silencioso `nodes[:200]`, `healthy=true` con error real en #205.) | **ALTA — RESUELTO** | Sonda `_fix_vcap`: operators_scanned=211, error_count=1, healthy=False, error paths=['n205'] |
...
| A3 | `GET /healthcheck` | **FIXED 2026-09-22**: el check es NO-MUTANTE por default (lee `errors()`/`warnings()` sin cocinar). `?force_cook=1` opt-in restaura el cook; la respuesta declara `forceCook` y cada ítem lleva `cooked` + `pre_existing_errors` (errores vistos ANTES del cook) para distinguir pre-existentes de materializados. Verificado en vivo: 3 nullTOPs limpios → default `ok=true, forceCook=false` y la red sigue limpia después; opt-in `forceCook=true, cooked=true, issueCount=3` (los errores que el cook materializa, ahora declarados). (Antes: `cook(force=True)` incondicional materializaba errores en nodos sanos.) | **ALTA — RESUELTO** (queda el matiz: opt-in sigue materializando, pero lo declara) | Sonda `_fix_hc`: default ok=True issueCount=0 forceCook=False, red limpia después; opt-in ok=False issueCount=3 forceCook=True |
| A4 | `POST /document` | **FIXED 2026-09-22**: la recursión es opt-in via `payload["recursive"]` (default 0 = solo hijos, shape histórico preservado) y la respuesta **declara** `recursive: true\|false`. Verificado en vivo: default `recursive=False, operator_count=1` con subCOMP anidado; `recursive=1` → `recursive=True, operator_count=2` (incluye lo anidado). Tests de frontera: `TestDocumentRecursion` (default, opt-in con error anidado visible, formas bool/str del flag). (Antes: solo hijos directos sin declararlo — "Network at X with 2 operators" falso.) | **MEDIA — RESUELTO** | Sandbox `_a4`: default recursive=False count=1; recursive=1 → True count=2 |
| A5 | `GET /metrics` | **FIXED 2026-09-22**: el cap de profundidad (30) ahora está **declarado** con claves aditivas: `max_depth` (cap), `walk_truncated` (¿se cortó el walk?) y `deepest_visited` (profundidad descartada, null si no hubo corte). Verificado en vivo con error real a profundidad 31: `walk_truncated=True, deepest_visited=31, max_depth=30`. Tests: frontera 31 niveles en `TestMetrics` + key-set en `TestMetricsContract`. (Antes: cap silencioso — error a 31 invisible sin aviso.) | **MEDIA — RESUELTO** (queda el matiz A2: errores no materializados) | Sandbox `_a5`: walk_truncated=True deepest_visited=31 |
...
**Recomendaciones (estado al 2026-09-22):**
1. ~~A1~~ **APLICADO**: scan completo + cap solo de serialización + `errors_truncated`/`warnings_truncated` aditivas.
2. ~~A3~~ **APLICADO**: no-cocinar por defecto; `?force_cook=1` opt-in con `forceCook`/`cooked`/`pre_existing_errors` en la respuesta.
3. ~~A4~~ **APLICADO**: `recursive` opt-in en el payload + flag declarado en la respuesta.
4. ~~A5~~ **APLICADO**: `max_depth` + `walk_truncated` + `deepest_visited` en `/metrics`.
