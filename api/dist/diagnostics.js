...
export function setClientLogPath(p) {
    logResolved = true;
    logPath = p;
    logDisabled = p === null;
    explicitLogPath = p;
 ...
 */
export function recordCall(rec) {
    ring.push(rec);
 ...
 */
export function classifyConnectionError(e) {
    ...
    if (/http \d{3}|http error \d{3}/.test(msg))
        return "http_error";
    ...
    if (/econnreset|socket hang up|connection dropped|epipe|other side closed|premature close|terminated|errno 104|connection reset|winerror 10054/.test(msg)) {
        return "connect_reset";
    }
    if (/econnrefused|enotfound|eaddrnotavail|ehostunreach|enetunreach|fetch failed|unable to connect|connection refused|could not connect|errno 111|winerror 10061|actively refused/.test(msg)) {
...
export function errorMessage(e, depth = 0) {
    if (depth > 3)
        return "";
...
export function isRetryable(kind, method) {
    const m = (method || "GET").toUpperCase();
    const isRead = m === "GET" || m === "HEAD";
    if (kind === "bridge_unreachable")
        return true; // never reached TD
...
export const KIND_HINTS = {
    bridge_unreachable: "El bridge de TD no está escuchando. Verificá que TD esté abierto, que el componente del API esté cargado y que el puerto sea el correcto (Settings → MCP). Si acabás de abrir TD, reintentá en unos segundos.",
    connect_reset: "La conexión se cortó con el socket ya abierto: TD puede haberse cerrado, congelado o quedado sin cocinar. Revisá el estado de TD antes de reintentar una escritura (pudo haberse aplicado).",
    timeout: "El bridge no respondió dentro del timeout. Suele ser TD ocupado u operación pesada: acotá el scope (path/limit) antes de reintentar.",
    http_error: "TD respondió con un error HTTP: es una respuesta real, no un problema de conexión. Leé el mensaje del bridge y corregí la llamada.",
    ws_error: "Falló el transporte WebSocket. Si el modo es 'auto' el cliente reintenta por HTTP; si es 'websocket', revisá que ese transporte esté disponible.",
