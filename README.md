...
- **Gating por evidencia de la matriz POP**: de los 97 tipos POP (TD 2025.31760), **78 están clasificados `ok_con_input`** (cocinan limpio CON fuente real y producen geometría — `docs/pop_matrix.json`); el planner y el catálogo de topología recomiendan esos por defecto y advierten sobre los 19 restantes (9 con `errors()`, 9 sin geometría, 1 no creable) salvo que el pedido los nombre explícitamente (`isRecommendedForNetworks` / `networkRecommendationWarning`).
